# Remove `packages/prioricode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/prioricode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the prioricode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/prioricode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/prioricode/src/account/repo.ts`
- `packages/prioricode/src/cli/cmd/db.ts`
- `packages/prioricode/src/cli/cmd/import.ts`
- `packages/prioricode/src/cli/cmd/stats.ts`
- `packages/prioricode/src/control-plane/workspace.ts`
- `packages/prioricode/src/index.ts`
- `packages/prioricode/src/node.ts`
- `packages/prioricode/src/permission/index.ts`
- `packages/prioricode/src/project/project.ts`
- `packages/prioricode/src/server/projectors.ts`
- `packages/prioricode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/prioricode/src/server/shared/fence.ts`
- `packages/prioricode/src/session/message-v2.ts`
- `packages/prioricode/src/session/projectors.ts`
- `packages/prioricode/src/session/prompt.ts`
- `packages/prioricode/src/session/session.ts`
- `packages/prioricode/src/session/todo.ts`
- `packages/prioricode/src/share/share-next.ts`
- `packages/prioricode/src/storage/db.ts`
- `packages/prioricode/src/sync/index.ts`
- `packages/prioricode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy prioricode database wrapper; `packages/prioricode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the prioricode wrapper.
...
Status: Completed. `SyncEvent` and the prioricode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy prioricode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy prioricode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy prioricode database wrapper.
...
All migration groups are complete or superseded. `packages/prioricode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No prioricode data-migration group remains.

The previous prioricode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate prioricode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/prioricode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/prioricode/src`
- `bun typecheck` from `packages/prioricode`
- Relevant package tests from `packages/prioricode`, not the repo root
