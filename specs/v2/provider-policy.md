...
Both `action` and `resource` use prioricode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.prioricode` files is intentionally deferred until `.prioricode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether prioricode permits an operation through its managed execution paths.
