...
The prioricode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@prioricode/llm` tool loop after replacing its
...
- plugins get global "prioricode" instance like in that post i showed
- prioricode instance has stuff like `prioricode.session.prompt()` or
  `prioricode.tool.register({...})`
...
- decide whether to preserve deprecated `@prioricode/llm` orchestration exports
