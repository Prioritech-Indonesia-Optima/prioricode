...
Move the canonical PrioriCode terminal application from
`packages/prioricode/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/prioricode`,
`packages/cli`, or `@prioricode/core`.

The SDK is the TUI's PrioriCode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/prioricode`.
- Do not replace private `packages/prioricode` imports with `@prioricode/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@prioricode/tui`.
...
- PrioriCode domain data displayed by the TUI
...
```text
packages/prioricode/src/cli/cmd/tui
```

Its private dependency on `packages/prioricode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/prioricode/src/*`.
...
Status: Completed. The private `@prioricode/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@prioricode/tui`.
...
- The package has no dependency on `prioricode`, `@prioricode/cli`, or
  `@prioricode/core`.
...
primitives now live in `@prioricode/tui`. The legacy host consumes explicit
...
and build inputs through `@prioricode/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads PrioriCode globals or process state;
...
Replace process-global PrioriCode state with resolved TUI inputs.
...
`@prioricode/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/prioricode` initially.
...
canonical in `@prioricode/tui`. Configured references resolve through the new
...
adapters rather than PrioriCode domain implementations.

Make the SDK the only PrioriCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports PrioriCode data and operations only from
  `@prioricode/sdk`.
...
`@prioricode/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/prioricode`.
...
- The package has no imports from `packages/prioricode`, `packages/cli`, or
  `@prioricode/core`.
...
public `@prioricode/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/prioricode`.
- Keep the legacy embedded worker and server startup in `packages/prioricode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@prioricode/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/prioricode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@prioricode/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@prioricode/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for PrioriCode domain state.
...
cd packages/prioricode && bun typecheck
...
rg '@prioricode/core|packages/prioricode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/prioricode/src packages/prioricode/test
```
...
- Build the current-platform `packages/prioricode` binary.
