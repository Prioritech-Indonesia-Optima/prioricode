...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "prioricode/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".prioricode/glossary/zh-cn.md"
  if (locale === "zht") return ".prioricode/glossary/zh-tw.md"
  return `.prioricode/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("PrioriCode did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("PrioriCode returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("PrioriCode session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel PrioriCode runs for 'all' (default: 4)
      --model <provider/id>  PrioriCode model (default: prioricode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running PrioriCode
...
`)
  ...
  if (failed.length) console.error(`\nPrioriCode failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.PRIORICODE_DISABLE_PROJECT_CONFIG = "1"
  env.PRIORICODE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["prioricode", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.PRIORICODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["prioricode", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.PRIORICODE_CONFIG
  delete env.PRIORICODE_CONFIG_DIR
  delete env.PRIORICODE_CONFIG_CONTENT
  delete env.PRIORICODE_PERMISSION
  delete env.PRIORICODE_AUTO_SHARE
  return env
}
