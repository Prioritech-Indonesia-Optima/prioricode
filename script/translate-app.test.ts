...
describe("translate app", () => {
  ...
  test("maps product locale codes to their glossaries", () => {
    expect(glossaryFile("fr")).toBe(".prioricode/glossary/fr.md")
    expect(glossaryFile("zh")).toBe(".prioricode/glossary/zh-cn.md")
    expect(glossaryFile("zht")).toBe(".prioricode/glossary/zh-tw.md")
  })
  ...
  test("reads the actual model and variant from the completed session", () => {
    ...
    ).toEqual([{ model: "prioricode/gpt-5.5", variant: "xhigh" }])
    expect(
      textFromEvents(
...
{"variants":{}}
`
    expect(modelVariants(output, "prioricode/gpt-5.5")).toEqual({
