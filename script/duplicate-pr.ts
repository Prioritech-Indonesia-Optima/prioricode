...
import { createPrioricode } from "@prioricode/sdk"
...
`)
  ...
  const prioricode = await createPrioricode({ port: 0 })
  ...
  try {
    ...
    const session = await prioricode.client.session.create()
    const result = await prioricode.client.session
      .prompt({
