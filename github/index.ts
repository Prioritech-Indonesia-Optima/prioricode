...
const { client, server } = createPrioricode()
...
try {
  ...
  await assertPrioricodeConnected()
  ...
  console.log("prioricode session", session.id)
  if (shareId) {
...
function createPrioricode() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`prioricode`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createPrioricodeClient({ baseUrl: url })
...
async function assertPrioricodeConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to prioricode server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.prioricode.ai" : "https://prioricode.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.prioricode.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("prioricode-github-action")
    response = await fetch("https://api.prioricode.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to prioricode...")
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `prioricode/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/prioricode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[prioricode session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
