...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve prioricode route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "prioricode")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "prioricode")).toBe("openai")
    expect(statProvider("big-pickle", "", "prioricode")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "prioricode"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep prioricode as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "prioricode"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "prioricode"))).toMatchObject([{ provider: "unknown" }])
    expect(toProviderAggregate(aggregate("muse-spark-1.2-contributor", "unknown"))).toMatchObject([
      { provider: "meta" },
    ])
  })

  test("geo aggregates never keep prioricode or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "prioricode"), country: "US" })).toMatchObject([
