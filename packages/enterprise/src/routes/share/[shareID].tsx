import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@prioricode/sdk/v2"
import { SessionTurn } from "@prioricode/session-ui/session-turn"
import { SessionReview } from "@prioricode/session-ui/session-review"
import { DataProvider } from "@prioricode/session-ui/context"
import { FileComponentProvider } from "@prioricode/ui/context/file"
import { WorkerPoolProvider } from "@prioricode/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@prioricode/ui/logo"
import { IconButton } from "@prioricode/ui/icon-button"
import { ProviderIcon } from "@prioricode/ui/provider-icon"
import { iife } from "@prioricode/core/util/iife"
import { Binary } from "@prioricode/core/util/binary"
import { NamedError } from "@prioricode/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@prioricode/ui/tabs"
import { MessageNav } from "@prioricode/session-ui/message-nav"
import { FileSSR } from "@prioricode/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@prioricode/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/prioricode-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | PrioriCode</Title>
              </Show>
              <Meta name="description" content="prioricode - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              ...
                              <IconButton
                                as={"a"}
                                href="https://prioricode.ai/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
