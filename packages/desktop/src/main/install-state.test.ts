...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier PrioriCode launch", () => {
    expect(hasExistingAppState([file("prioricode.settings")])).toBe(true)
    expect(hasExistingAppState([file("prioricode.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("prioricode")])).toBe(true)
  })
})
