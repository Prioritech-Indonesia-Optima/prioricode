...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-prioricode", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installPrioricode(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-prioricode", unavailable)
