import type { WslDistroProbe, WslPrioricodeCheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  prioricodeChecks: Record<string, WslPrioricodeCheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextPrioricodeChecks = { ...prioricodeChecks }
  delete nextDistroProbes[distro]
  delete nextPrioricodeChecks[distro]
  return { distroProbes: nextDistroProbes, prioricodeChecks: nextPrioricodeChecks }
}
