...
let releasePrioricodeResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectPrioricodeVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectPrioricodeVersion("1.14.35", "1.16.2")).toThrow(
    "PrioriCode update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating PrioriCode", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.prioricode/bin/prioricode",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, prioricodeChecks: {} })
})
...
test("ignores stale background PrioriCode checks after removing a WSL server", async () => {
  persistedServers = []
  releasePrioricodeResolve = undefined
  ...
  await waitFor(() => !!releasePrioricodeResolve)
  await controller.removeServer("wsl:Debian")
  releasePrioricodeResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().prioricodeChecks).toEqual({})
})

test("ignores stale startup PrioriCode checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking PrioriCode", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const prioricode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolvePrioricode: async (distro) => {
      prioricode.push(distro)
      return "/home/me/.prioricode/bin/prioricode"
    },
  })
  ...
  expect(prioricode).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(prioricode).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().prioricodeChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check PrioriCode in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const prioricode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(prioricode).toEqual(["Debian"])
  expect(Object.keys(controller.getState().prioricodeChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolvePrioricode: async () => {
      await new Promise<void>((resolve) => {
        releasePrioricodeResolve = resolve
      })
      return "/home/me/.prioricode/bin/prioricode"
    },
  }
