...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export PRIORICODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export PRIORICODE_CLIENT=desktop",
    `export PRIORICODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export PRIORICODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(prioricode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
