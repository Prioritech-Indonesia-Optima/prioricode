...
const TEST_ONBOARDING = process.env.PRIORICODE_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = process.env.PRIORICODE_SIDECAR_V2 === "1" ? "v2" : "v1"
...
const main = Effect.gen(function* () {
  ...
  process.env.PRIORICODE_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.prioricode.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `prioricode-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "PrioriCode Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("prioricode://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("prioricode")
  registerRendererProtocol()
  setDockIcon()
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.PRIORICODE_PORT
      if (fromEnv) {
