...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.prioricode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `prioricode.settings.dat` is special: it maps to the `prioricode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"prioricode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // prioricode.settings.dat → the electron settings store ("prioricode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "prioricode.global.dat").
  const storeName = filename === "prioricode.settings.dat" ? "prioricode.settings" : filename
