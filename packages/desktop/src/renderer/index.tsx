    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.PRIORICODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "prioricode:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__PRIORICODE__ ??= {}
  const pending = window.__PRIORICODE__.deepLinks ?? []
  window.__PRIORICODE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `prioricode.desktop.window.${windowID}.last-active-url`
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("prioricode.global.dat").getItem("language")
