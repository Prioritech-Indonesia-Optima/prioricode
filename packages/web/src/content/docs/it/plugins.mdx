---
title: Plugin
description: Scrivi plugin per estendere PrioriCode.
---

I plugin ti permettono di estendere PrioriCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di PrioriCode.
...
- `.prioricode/plugins/` - plugin a livello progetto
- `~/.config/prioricode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/prioricode/node_modules/`.
...
1. Config globale (`~/.config/prioricode/prioricode.json`)
2. Config di progetto (`prioricode.json`)
3. Directory plugin globale (`~/.config/prioricode/plugins/`)
4. Directory plugin di progetto (`.prioricode/plugins/`)
...
PrioriCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di prioricode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere prioricode.
...
:::note
Se usi l'app desktop di PrioriCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a prioricode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a prioricode:
...
L'helper `tool` crea uno strumento personalizzato che prioricode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in prioricode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://prioricode.ai/docs/sdk) per i dettagli.
