---
title: Server
description: Interagisci con il server prioricode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `prioricode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client prioricode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `prioricode.local` |
...
Imposta `PRIORICODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `prioricode`, oppure imposta `PRIORICODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `prioricode serve` sia per `prioricode web`.
...
Quando esegui `prioricode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server prioricode per interagire con prioricode in modo programmatico.
:::

Questa architettura permette a prioricode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `prioricode serve` per avviare un server standalone. Se la TUI di prioricode e gia in esecuzione, `prioricode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di PrioriCode.
...
Il server prioricode espone le seguenti API.
