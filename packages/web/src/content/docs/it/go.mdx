...
PrioriCode Go è un abbonamento a basso costo da **10 $/mese** che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in PrioriCode. Ti abboni a PrioriCode Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare PrioriCode.
...
PrioriCode Go ti dà accesso a questi modelli a **10 $/mese**.

---

## Come funziona

PrioriCode Go funziona come qualsiasi altro provider in PrioriCode.

1. Accedi a **<a href={console}>PrioriCode Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `PrioriCode Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a PrioriCode Go.
:::
...
PrioriCode Go è progettato per [PrioriCode](https://prioricode.ai) e altri agenti di programmazione
...
3. Inviare un ID di sessione stabile in `x-prioricode-session` per ogni conversazione, in modo da consentirci di ottimizzare il routing e
   il caching dei prompt.

### Client convalidati

Oltre a PrioriCode, è stato verificato che i seguenti client funzionino correttamente
con PrioriCode Go. Tuttavia, non garantiamo che continueranno a funzionare in futuro.

| Client            | Supporto delle sessioni                                                                                                                                                                                                                                                      |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | Le build contenenti la [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) inviano l'header nelle richieste PrioriCode principali e ausiliarie. La correzione è stata unita dopo la v0.21.0; quella versione da sola non la include.                        |
...
| **ZCode**         | Go riconosce il suo header di sessione nativo. La nostra [richiesta per `x-prioricode-session`](https://github.com/zai-org/feedback/issues/492) è ancora aperta, ma non è più necessario inviare proprio quell'header.                                                         |
| **Pi**            | Le build attuali inviano le informazioni di sessione per PrioriCode. Aggiorna le installazioni meno recenti.                                                                                                                                                                   |
...
| **Kilo Code CLI** | Le build contenenti la [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) ripristinano gli header di sessione PrioriCode. Questa correzione riguarda la CLI, non l'estensione VS Code. Consulta l'[issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
Il [model id](/docs/config/#models) nella tua PrioriCode config
utilizza il formato `prioricode-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`prioricode-go/kimi-k3` nella tua configurazione.
...
Abbiamo creato PrioriCode Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con PrioriCode.
