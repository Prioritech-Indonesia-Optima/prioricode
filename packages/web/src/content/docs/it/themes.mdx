...
Con PrioriCode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, PrioriCode usa il tema `prioricode`.
...
PrioriCode include diversi temi integrati.
...
- Vuole che PrioriCode corrisponda all'aspetto del terminale
...
PrioriCode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/prioricode/themes/*.json` o `$XDG_CONFIG_HOME/prioricode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.prioricode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.prioricode/themes/*.json`
