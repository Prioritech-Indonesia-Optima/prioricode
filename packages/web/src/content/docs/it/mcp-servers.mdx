...
Puoi aggiungere strumenti esterni a PrioriCode usando il _Model Context Protocol_, o MCP. PrioriCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [PrioriCode Config](https://prioricode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/prioricode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
PrioriCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, PrioriCode:
...
Se il server richiede autenticazione, PrioriCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `prioricode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, PrioriCode memorizzera' i token in modo sicuro in `~/.local/share/prioricode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in PrioriCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione PrioriCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare PrioriCode al tuo account Sentry.
