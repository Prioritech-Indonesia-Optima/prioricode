...
Le skill degli agenti permettono a PrioriCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
PrioriCode cerca in queste posizioni:

- Config di progetto: `.prioricode/skills/<name>/SKILL.md`
- Config globale: `~/.config/prioricode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, PrioriCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.prioricode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/prioricode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.prioricode/skills/git-release/SKILL.md` cosi':
...
PrioriCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `prioricode.json`:
...
**Per agenti integrati** (in `prioricode.json`):
