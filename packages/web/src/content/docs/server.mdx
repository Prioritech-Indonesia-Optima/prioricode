---
title: Server
description: Interact with prioricode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `prioricode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an prioricode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `prioricode.local` |
...
Set `PRIORICODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `prioricode`, or set `PRIORICODE_SERVER_USERNAME` to override it. This applies to both `prioricode serve` and `prioricode web`.
...
When you run `prioricode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the prioricode server to interact with prioricode programmatically.
:::

This architecture lets prioricode support multiple clients and allows you to interact with prioricode programmatically.

You can run `prioricode serve` to start a standalone server. If you have the
prioricode TUI running, `prioricode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the PrioriCode [IDE](/docs/ide) plugins.
...
The prioricode server exposes the following APIs.
