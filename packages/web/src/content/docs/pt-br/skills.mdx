...
As habilidades do agente permitem que o prioricode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O prioricode pesquisa nesses locais:

- Configuração do projeto: `.prioricode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/prioricode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o prioricode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.prioricode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/prioricode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.prioricode/skills/git-release/SKILL.md` assim:
...
O prioricode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `prioricode.json`:
...
**Para agentes embutidos** (em `prioricode.json`):
