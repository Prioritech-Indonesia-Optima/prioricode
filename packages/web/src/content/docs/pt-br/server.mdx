---
title: Servidor
description: Interaja com o servidor prioricode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `prioricode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente prioricode pode usar.
...
```bash
prioricode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `prioricode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`               |

`--cors` pode ser passado várias vezes:

```bash
prioricode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `PRIORICODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `prioricode`, ou defina `PRIORICODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `prioricode serve` quanto ao `prioricode web`.

```bash
PRIORICODE_SERVER_PASSWORD=your-password prioricode serve
```
...
Quando você executa `prioricode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor prioricode para interagir com o prioricode programaticamente.
:::

Essa arquitetura permite que o prioricode suporte múltiplos clientes e permite que você interaja com o prioricode programaticamente.

Você pode executar `prioricode serve` para iniciar um servidor autônomo. Se você tiver o TUI do prioricode em execução, `prioricode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do prioricode [IDE](/docs/ide).
...
O servidor prioricode expõe as seguintes APIs.
