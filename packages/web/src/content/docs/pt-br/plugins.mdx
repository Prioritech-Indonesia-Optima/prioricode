---
title: Plugins
description: Escreva seus próprios plugins para estender o prioricode.
---

Plugins permitem que você estenda o prioricode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do prioricode.
...
- `.prioricode/plugins/` - Plugins em nível de projeto
- `~/.config/prioricode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/prioricode/node_modules/`.
...
1. Configuração global (`~/.config/prioricode/prioricode.json`)
2. Configuração do projeto (`prioricode.json`)
3. Diretório de plugins global (`~/.config/prioricode/plugins/`)
4. Diretório de plugins do projeto (`.prioricode/plugins/`)
...
O prioricode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do prioricode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o prioricode.
...
:::note
Se você estiver usando o aplicativo desktop prioricode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o prioricode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao prioricode:
...
O helper `tool` cria uma ferramenta personalizada que o prioricode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o prioricode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://prioricode.ai/docs/sdk) para detalhes.
