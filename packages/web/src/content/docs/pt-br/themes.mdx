...
Com o prioricode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o prioricode usa nosso próprio tema `prioricode`.
...
O prioricode vem com vários temas embutidos.
...
- Querem que o prioricode corresponda à aparência do seu terminal
...
O prioricode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/prioricode/themes/*.json` ou `$XDG_CONFIG_HOME/prioricode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.prioricode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.prioricode/themes/*.json`
