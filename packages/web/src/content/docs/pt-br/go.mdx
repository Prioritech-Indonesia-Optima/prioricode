...
O PrioriCode Go é uma assinatura de baixo custo de **US$ 10/mês** que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no PrioriCode. Você assina o PrioriCode Go e
...
O PrioriCode Go lhe dá acesso a esses modelos por **US$ 10/mês**.

---

## Como funciona

O PrioriCode Go funciona como qualquer outro provedor no PrioriCode.

1. Você entra no **<a href={console}>PrioriCode Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `PrioriCode Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o PrioriCode Go.
:::
...
O PrioriCode Go foi projetado para o [PrioriCode](https://prioricode.ai) e outros agentes de programação
...
3. Enviar um ID de sessão estável em `x-prioricode-session` para cada conversa, para que possamos otimizar o roteamento e
   o cache de prompts.

### Clientes validados

Além do PrioriCode, os clientes a seguir foram validados para funcionar corretamente
com o PrioriCode Go. No entanto, não garantimos que continuarão funcionando no futuro.

| Cliente           | Suporte a sessões                                                                                                                                                                                                                                                       |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | As builds que contêm o [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) enviam o header nas requisições principais e auxiliares do PrioriCode. A correção foi incorporada após a v0.21.0; essa versão por si só não a inclui.                     |
...
| **ZCode**         | O Go reconhece seu header de sessão nativo. Nossa [solicitação de `x-prioricode-session`](https://github.com/zai-org/feedback/issues/492) continua aberta, mas não é mais necessário enviar especificamente esse header.                                                |
| **Pi**            | As builds atuais enviam informações de sessão para o PrioriCode. Atualize instalações mais antigas.                                                                                                                                                                     |
...
| **Kilo Code CLI** | As builds que contêm o [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) restauram os headers de sessão do PrioriCode. Essa correção abrange a CLI, não a extensão do VS Code. Consulte a [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
O [ID do modelo](/docs/config/#models) na sua configuração do PrioriCode
usa o formato `prioricode-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`prioricode-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o PrioriCode também.
