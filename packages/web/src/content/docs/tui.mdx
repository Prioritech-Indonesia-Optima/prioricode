---
title: TUI
description: Using the PrioriCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

PrioriCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running PrioriCode starts the TUI for the current directory.
...
When using the PrioriCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to PrioriCode. Allows you to select from available providers and add their API keys.
...
Exit PrioriCode. _Aliases_: `/quit`, `/q`
...
This is separate from `prioricode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long PrioriCode waits after the leader key. Defaults to `2000`.
...
Use `PRIORICODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `prioricode.default`.
