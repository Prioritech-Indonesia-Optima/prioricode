---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um PrioriCode zu erweitern.
---

Mit Plugins können Sie PrioriCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von PrioriCode zu ändern.
...
- `.prioricode/plugins/` – Plugins auf Projektebene
- `~/.config/prioricode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/prioricode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/prioricode/prioricode.json`)
2. Projektkonfiguration (`prioricode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/prioricode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.prioricode/plugins/`)
...
PrioriCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein PrioriCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie PrioriCode erweitern können.
...
:::note
Wenn Sie die PrioriCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass PrioriCode `.env`-Dateien liest:
...
Plugins können PrioriCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das PrioriCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für PrioriCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://prioricode.ai/docs/sdk).
