...
Mit PrioriCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet PrioriCode das `prioricode`-Theme.
...
PrioriCode liefert mehrere eingebaute Themes mit.
...
- PrioriCode optisch ans Terminal anpassen wollen
...
PrioriCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/prioricode/themes/*.json` oder `$XDG_CONFIG_HOME/prioricode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.prioricode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.prioricode/themes/*.json`
