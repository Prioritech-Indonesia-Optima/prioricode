...
PrioriCode Go ist ein kostengünstiges Abonnement für **10 $/Monat**, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in PrioriCode. Du abonnierst PrioriCode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
PrioriCode zu verwenden.
...
PrioriCode Go bietet dir Zugriff auf diese Modelle für **10 $/Monat**.

---

## Wie es funktioniert

PrioriCode Go funktioniert wie jeder andere Provider in PrioriCode.

1. Du meldest dich bei **<a href={console}>PrioriCode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `PrioriCode Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann PrioriCode Go abonnieren.
:::
...
PrioriCode Go wurde für [PrioriCode](https://prioricode.ai) und andere Coding-Agenten entwickelt,
...
3. für jede Unterhaltung eine stabile Sitzungs-ID in `x-prioricode-session` senden, damit wir das Routing und
   Prompt-Caching optimieren können.

### Validierte Clients

Neben PrioriCode wurden die folgenden Clients für die ordnungsgemäße Verwendung mit
PrioriCode Go validiert. Wir garantieren jedoch nicht, dass sie auch in Zukunft funktionieren werden.

| Client            | Sitzungsunterstützung                                                                                                                                                                                                                                           |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | Builds mit [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) senden den Header bei Haupt- und zusätzlichen PrioriCode-Anfragen. Der Fix wurde nach v0.21.0 gemergt; diese Version allein enthält ihn nicht.                                |
...
| **ZCode**         | Go erkennt seinen nativen Sitzungs-Header. Unsere [Anfrage für `x-prioricode-session`](https://github.com/zai-org/feedback/issues/492) ist weiterhin offen, aber es ist nicht mehr erforderlich, genau diesen Header zu senden.                                 |
| **Pi**            | Aktuelle Builds senden Sitzungsinformationen für PrioriCode. Aktualisiere ältere Installationen.                                                                                                                                                                |
...
| **Kilo Code CLI** | Builds mit [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) stellen PrioriCode-Sitzungs-Header wieder her. Dieser Fix gilt für die CLI, nicht für die VS-Code-Erweiterung. Siehe [Issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
Die [Modell-ID](/docs/config/#models) in deiner PrioriCode Config verwendet das Format `prioricode-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `prioricode-go/kimi-k3` in deiner Config verwenden.
...
Wir haben PrioriCode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit PrioriCode zu nutzen.
