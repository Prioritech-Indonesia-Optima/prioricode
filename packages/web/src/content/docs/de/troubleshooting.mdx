...
Wenn PrioriCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\prioricode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `prioricode --log-level DEBUG`.

---

## Speicher

prioricode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/prioricode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\prioricode`
...
PrioriCode Desktop startet im Hintergrund einen lokalen PrioriCode-Server (`prioricode-cli`-Sidecar).
...
- Nur macOS: `PrioriCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/prioricode/prioricode.jsonc` (oder `~/.config/prioricode/prioricode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/prioricode/prioricode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\prioricode\prioricode.jsonc`
...
PrioriCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/prioricode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\prioricode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit PrioriCode ihn neu aufbauen kann.

1. Beende PrioriCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/prioricode`
- **Linux**: loesche `~/.cache/prioricode` (oder fuehre aus: `rm -rf ~/.cache/prioricode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\prioricode`

3. Starte PrioriCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

PrioriCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `prioricode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `PRIORICODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `PRIORICODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt PrioriCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit PrioriCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

PrioriCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer PrioriCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende PrioriCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von PrioriCode Desktop):

- `prioricode.settings.dat` (desktop default server URL)
- `prioricode.global.dat` und `prioricode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit PrioriCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `prioricode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\prioricode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. prioricode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\prioricode`

2. Starte prioricode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt prioricode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
prioricode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
