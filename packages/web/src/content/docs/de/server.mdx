---
title: Server
description: Nutze den prioricode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `prioricode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein prioricode-Client nutzen kann.
...
Setze `PRIORICODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `prioricode`, kann aber mit `PRIORICODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `prioricode serve` und `prioricode web`.
...
Wenn du `prioricode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den prioricode-Server, um prioricode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `prioricode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `prioricode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die PrioriCode-[IDE](/docs/ide)-Plugins.
...
Der prioricode-Server stellt folgende APIs bereit.
