---
title: Regeln
description: Lege eigene Anweisungen fuer prioricode fest.
---

Du kannst prioricode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in prioricode den Befehl `/init` aus.
...
So kann prioricode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht prioricode sie zu erweitern.
...
prioricode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/prioricode/AGENTS.md` speichern.
Diese gelten in allen prioricode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt PrioriCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/prioricode/AGENTS.md` existiert)
...
Beim Start sucht prioricode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/prioricode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `prioricode.json` oder in `~/.config/prioricode/prioricode.json` angeben.
...
prioricode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit prioricode.json

Empfohlen ist das `instructions`-Feld in der `prioricode.json`:
...
Du kannst prioricode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `prioricode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
