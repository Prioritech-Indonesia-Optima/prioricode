...
Agent Skills erlauben PrioriCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
PrioriCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft PrioriCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.prioricode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/prioricode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.prioricode/skills/git-release/SKILL.md` zum Beispiel so:
...
PrioriCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `prioricode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `prioricode.json`):
