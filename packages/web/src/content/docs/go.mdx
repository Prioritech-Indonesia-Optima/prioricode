...
PrioriCode Go is a low cost **$10/month subscription** that gives you reliable access to popular open coding models.

Go works like any other provider in PrioriCode. You subscribe to PrioriCode Go and
...
PrioriCode Go gives you access to these models for **$10/month**.

---

## How it works

PrioriCode Go works like any other provider in PrioriCode.

1. You sign in to **<a href={console}>PrioriCode Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `PrioriCode Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to PrioriCode Go.
:::
...
PrioriCode Go is designed for [PrioriCode](https://prioricode.ai) and other coding agents
...
3. Send a stable session ID in `x-prioricode-session` for each conversation so we can optimize routing and
   prompt caching.

### Validated Clients

Besides PrioriCode, the following clients have been validated to work properly
with PrioriCode Go. Although we do not guarantee that they will continue to work in the future.

| Client            | Session support                                                                                                                                                                                                                               |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | Builds containing [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) send the header on main and auxiliary PrioriCode requests. The fix was merged after v0.21.0; that release alone does not include it.                 |
...
| **ZCode**         | Go recognizes its native session header. Our [request for `x-prioricode-session`](https://github.com/zai-org/feedback/issues/492) remains open, but it is no longer necessary to send that specific header.                                   |
| **Pi**            | Current builds send session information for PrioriCode. Update older installations.                                                                                                                                                           |
| **jcode**         | Update to **v0.81.6 or later**, which includes the [session-header fix](https://github.com/1jehuang/jcode/issues/1167).                                                                                                                       |
| **Kilo Code CLI** | Builds containing [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) restore PrioriCode session headers. This fix covers the CLI, not the VS Code extension. See [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
uses the format `prioricode-go/<model-id>`. For example, for Kimi K3, you would
use `prioricode-go/kimi-k3` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with PrioriCode as well.
