---
title: Plugins
description: Skriv dine egne plugins for at udvide PrioriCode.
---

Plugins giver dig mulighed for at udvide PrioriCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre PrioriCodes standardadfærd.
...
- `.prioricode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/prioricode/node_modules/`.
...
1. Global konfiguration (`~/.config/prioricode/prioricode.json`)
2. Projektkonfiguration (`prioricode.json`)
3. Global plugin-mappe (`~/.config/prioricode/plugins/`)
4. Projekt plugin bibliotek (`.prioricode/plugins/`)
...
PrioriCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En prioricode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide prioricode.
...
:::note
Hvis du bruger PrioriCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå prioricode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til prioricode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som prioricode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for prioricode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://prioricode.ai/docs/sdk) for detaljer.
