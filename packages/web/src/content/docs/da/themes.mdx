...
Med PrioriCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger PrioriCode vårt eget `prioricode`-tema.
...
PrioriCode kommer med flere innebygde temaer.
...
- Vil at PrioriCode skal matche terminalens utseende
...
PrioriCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/prioricode/themes/*.json` eller `$XDG_CONFIG_HOME/prioricode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.prioricode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.prioricode/themes/*.json`
