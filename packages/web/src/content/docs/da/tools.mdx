...
Her er alle de innebygde verktøyene som er tilgængelige i PrioriCode.
...
:::note
Dette verktøyet er bare tilgængelig når `PRIORICODE_EXPERIMENTAL_LSP_TOOL=true` (eller `PRIORICODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger PrioriCode-leverandøren eller når miljøvariabelen `PRIORICODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
