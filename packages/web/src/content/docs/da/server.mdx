---
title: Server
description: Interager med prioricode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `prioricode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en prioricode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `prioricode.local` |
...
Indstil `PRIORICODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `prioricode`, eller indstil `PRIORICODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `prioricode serve` og `prioricode web`.
...
Når du kører `prioricode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug prioricode-serveren til at interagere med prioricode programmatisk.
:::

Denne arkitektur lader prioricode understøtte flere klienter og lader dig interagere med prioricode programmatisk.

Du kan køre `prioricode serve` for at starte en selvstændig server. Hvis du har
prioricode TUI kørende, vil `prioricode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af PrioriCode [IDE](/docs/ide) plugins.
...
PrioriCode-serveren viser følgende API'er.
