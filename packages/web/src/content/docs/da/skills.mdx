...
Agentferdigheter lar PrioriCode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
PrioriCode søker etter disse stedene:

- Prosjektkonfigurasjon: `.prioricode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/prioricode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går PrioriCode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.prioricode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/prioricode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.prioricode/skills/git-release/SKILL.md` slik:
...
PrioriCode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `prioricode.json`:
...
**For innebygde agenter** (i `prioricode.json`):
