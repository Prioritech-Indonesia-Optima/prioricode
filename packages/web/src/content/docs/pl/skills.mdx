...
Umiejętności agenta pozwalają prioricode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
prioricode przeszukuje te lokalizacje:

- Project config: `.prioricode/skills/<name>/SKILL.md`
- Global config: `~/.config/prioricode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu prioricode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.prioricode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/prioricode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.prioricode/skills/git-release/SKILL.md` w ten sposób:
...
prioricode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `prioricode.json`:
...
**Dla agentów wbudowanych** (w `prioricode.json`):
