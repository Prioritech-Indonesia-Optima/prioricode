---
title: Serwer
description: Interakcja z serwerem prioricode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `prioricode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient prioricode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `prioricode.local` |
...
Ustaw `PRIORICODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `prioricode` lub ustaw `PRIORICODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `prioricode serve`, jak i `prioricode web`.
...
Kiedy uruchomisz `prioricode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera prioricode do programowej interakcji z prioricode.
:::

Ta architektura umożliwia obsługę wielu klientów przez prioricode i programową interakcję z kodem otwartym.

Możesz uruchomić `prioricode serve`, aby uruchomić samodzielny serwer. Jeśli masz
prioricode TUI działa, `prioricode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki prioricode [IDE](/docs/ide).
...
Serwer prioricode udostępnia następujące interfejsy API.
