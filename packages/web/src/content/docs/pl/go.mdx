...
PrioriCode Go to niskokosztowa subskrypcja za **10 $/miesiąc**, która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w PrioriCode. Subskrybujesz PrioriCode Go i
...
PrioriCode Go daje Ci dostęp do tych modeli za **10 $/miesiąc**.

---

## Jak to działa

PrioriCode Go działa jak każdy inny dostawca w PrioriCode.

1. Logujesz się do **<a href={console}>PrioriCode Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `PrioriCode Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować PrioriCode Go.
:::
...
PrioriCode Go jest przeznaczony dla [PrioriCode](https://prioricode.ai) i innych agentów kodujących,
...
3. Wysyłać stabilny identyfikator sesji w nagłówku `x-prioricode-session` dla każdej rozmowy, aby umożliwić optymalizację routingu i
   buforowania promptów.

### Zweryfikowane klienty

Poza PrioriCode zweryfikowano, że poniżsi klienci działają prawidłowo
z PrioriCode Go. Nie gwarantujemy jednak, że będą nadal działać w przyszłości.

| Klient            | Obsługa sesji                                                                                                                                                                                                                                                |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Hermes**        | Kompilacje zawierające [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) wysyłają nagłówek w głównych i pomocniczych żądaniach PrioriCode. Poprawkę scalono po wersji v0.21.0; samo to wydanie jej nie zawiera.                         |
...
| **ZCode**         | Go rozpoznaje jego natywny nagłówek sesji. Nasza [prośba o `x-prioricode-session`](https://github.com/zai-org/feedback/issues/492) pozostaje otwarta, ale wysyłanie tego konkretnego nagłówka nie jest już konieczne.                                        |
| **Pi**            | Aktualne kompilacje wysyłają informacje o sesji dla PrioriCode. Zaktualizuj starsze instalacje.                                                                                                                                                              |
...
| **Kilo Code CLI** | Kompilacje zawierające [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) przywracają nagłówki sesji PrioriCode. Ta poprawka obejmuje CLI, ale nie rozszerzenie VS Code. Zobacz [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji PrioriCode
używa formatu `prioricode-go/<model-id>`. Na przykład dla Kimi K3 należy użyć
`prioricode-go/kimi-k3` w swojej konfiguracji.
...
Stworzyliśmy PrioriCode Go, aby:
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z PrioriCode.
