...
Możesz dodać dodatkowe informacje do prioricode za pomocą _Model Context Protocol_, w skrócie MCP. prioricode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [prioricode Config](https://prioricode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/prioricode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
prioricode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, prioricode:
...
Jeśli serwer wymaga uwierzytelnienia, prioricode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `prioricode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji prioricode bezpieczne przechowa tokeny w `~/.local/share/prioricode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w prioricode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie prioricode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć prioricode z kontem Sentry.
