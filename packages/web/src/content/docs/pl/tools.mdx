...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. PrioriCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w PrioriCode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `PRIORICODE_EXPERIMENTAL_LSP_TOOL=true` (lub `PRIORICODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy PrioriCode lub gdy zmienna środowiskowa `PRIORICODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu PrioriCode:
