...
Dzięki prioricode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie prioricode używa naszego własnego motywu `prioricode`.
...
prioricode ma kilka wbudowanych motywów.
...
- Chcą, aby PrioriCode pasował do wyglądu ich terminala
...
prioricode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/prioricode/themes/*.json` lub `$XDG_CONFIG_HOME/prioricode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.prioricode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.prioricode/themes/*.json`
