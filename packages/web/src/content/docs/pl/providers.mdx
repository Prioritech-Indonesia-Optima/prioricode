---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w prioricode.
---

import config from "../../../../config.mjs"
export const console = config.console

prioricode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji prioricode.
...
w `~/.local/share/prioricode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim prioricode
config.
...
PrioriCode Zen to lista modeli dostarczonych przez zespół prioricode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z prioricode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od PrioriCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `PrioriCode Zen` i przejdź do [prioricode.ai/auth](https://prioricode.ai/zen).
...
Działa jak każdy inny dostawca w prioricode i jest całkowicie opcjonalny w użyciu.

---

## PrioriCode Go

PrioriCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół prioricode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z prioricode.

1. Uruchom polecenie `/connect` w TUI, wybierz `PrioriCode Go` i przejdź do [prioricode.ai/auth](https://prioricode.ai/zen).
...
Aby używać Amazon Bedrock z prioricode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania prioricode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `prioricode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w prioricode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować prioricode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod prioricode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji prioricode.
...
prioricode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować prioricode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`prioricode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem prioricode:
...
Aby używać Google Vertex AI z prioricode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania prioricode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą prioricode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [prioricode-helicone-session](https://github.com/H2Shami/prioricode-helicone-session), aby automatycznie rejestrować każdą konwersację prioricode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja prioricode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować prioricode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować prioricode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować prioricode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem prioricode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/prioricode).
:::
...
Aby korzystać z Ollama Cloud z prioricode:
...
3. Skopiuj klucz API do użycia w prioricode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w prioricode musisz pobrać informacje o modelu lokalnie:
...
PrioriCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół prioricode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>PrioriCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **PrioriCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API prioricode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji prioricode.
...
5. Możesz je także dostosować za pomocą konfiguracji prioricode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z prioricode:
...
Możesz także dostosować modele za pomocą konfiguracji prioricode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z prioricode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `prioricode.json` w katalogu projektu:
...
Pola `limit` pozwalają prioricode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `prioricode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację prioricode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji prioricode.
