...
Создайте `.prioricode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию prioricode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем prioricode [config](/docs/config):
...
- Глобальный: `~/.config/prioricode/commands/`
- Для каждого проекта: `.prioricode/commands/`
...
prioricode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
