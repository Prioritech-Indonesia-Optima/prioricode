...
ツールを使用すると、LLM がコードベースでアクションを実行できるようになります。 PrioriCode には一連の組み込みツールが付属していますが、[カスタムツール](/docs/custom-tools) または [MCP サーバー](/docs/mcp-servers) を使用して拡張できます。
...
PrioriCode で利用可能なすべての組み込みツールを次に示します。
...
:::note
このツールは、`PRIORICODE_EXPERIMENTAL_LSP_TOOL=true` (または `PRIORICODE_EXPERIMENTAL=true`) の場合にのみ使用できます。
:::
...
:::note
このツールは、PrioriCode プロバイダーを使用している場合、または `PRIORICODE_ENABLE_EXA` 環境変数が真実の値 (`true` または `1` など) に設定されている場合にのみ使用できます。

PrioriCode の起動時に有効にするには:
