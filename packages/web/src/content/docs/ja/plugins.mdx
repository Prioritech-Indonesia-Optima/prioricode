---
title: プラグイン
description: PrioriCode を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで PrioriCode を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、PrioriCode のデフォルトの動作を変更したりできます。
...
- `.prioricode/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/prioricode/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/prioricode/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/prioricode/prioricode.json`)
2. プロジェクト設定 (`prioricode.json`)
3. グローバルプラグインディレクトリ (`~/.config/prioricode/plugins/`)
4. プロジェクトプラグインディレクトリ (`.prioricode/plugins/`)
...
PrioriCode は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための PrioriCode SDK クライアント。
...
ここでは、PrioriCode を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://prioricode.ai/docs/sdk) を参照してください。
...
PrioriCode が `.env` ファイルを読み取らないようにします。
...
プラグインは PrioriCode にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、PrioriCode が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して PrioriCode で使用できます。
