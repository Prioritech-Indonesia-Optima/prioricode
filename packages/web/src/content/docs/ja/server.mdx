---
title: サーバー
description: HTTP 経由で PrioriCode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`prioricode serve` コマンドは、PrioriCode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `prioricode.local` |
...
HTTP 基本認証でサーバーを保護するには、`PRIORICODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `prioricode` になるか、`PRIORICODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`prioricode serve` と `prioricode web` の両方に当てはまります。
...
`prioricode` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
PrioriCode サーバーを使用して、プログラムで PrioriCode と対話します。
:::

`prioricode serve` を実行してスタンドアロンサーバーを起動できます。すでに PrioriCode TUI を実行している場合でも、`prioricode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、PrioriCode [IDE](/docs/ide) プラグインによって使用されます。
