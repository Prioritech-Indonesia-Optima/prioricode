---
title: Policies
description: Control which configured resources PrioriCode may use.
---

Policies control whether PrioriCode may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `prioricode.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether PrioriCode may use a resource such as an LLM provider.
...
PrioriCode currently supports one policy action:
