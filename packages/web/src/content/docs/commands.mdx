...
Create `.prioricode/commands/test.md`:
...
You can add custom commands through the PrioriCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your PrioriCode [config](/docs/config):
...
- Per-project: `.prioricode/commands/`
...
prioricode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
