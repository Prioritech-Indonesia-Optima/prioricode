...
There are two types of agents in PrioriCode; primary agents and subagents.
...
PrioriCode comes with three built-in primary agents, **Build**, **Plan**, and **Auto**. We'll
look at these below.
...
PrioriCode comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

PrioriCode comes with three built-in primary agents and three built-in subagents.
...
A fully autonomous agent for hands-off work. It runs the entire cycle on its own — explore, brainstorm, plan, execute, verify, and report — without pausing to ask you questions. The `question` tool is denied, so it makes decisions autonomously and picks the safest, most reversible option when uncertain.

It writes a plan to a plan file before executing, then works through it phase by phase, running each phase's verification command and **auto-committing** after every verified phase (commit message `auto: <phase name>`). This gives you per-phase rollback points to inspect later.

Use it when you want to kick off a task and step away — it is designed for AFK use.

```bash frame="none" title="Start a session in Auto mode"
prioricode --agent auto "add rate limiting to the /api/login endpoint"
```
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into PrioriCode's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `prioricode.json` config file:
...
- Global: `~/.config/prioricode/agents/`
- Per-project: `.prioricode/agents/`

```markdown title="~/.config/prioricode/agents/review.md"
---
...
If no temperature is specified, PrioriCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global PrioriCode config and the project specific config.
...
The model ID in your PrioriCode config uses the format `provider/model-id`. For example, if you're using [PrioriCode Zen](/docs/zen), you would use `prioricode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="prioricode.json" {3-6,9-12}
{
  "$schema": "https://prioricode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="prioricode.json" {3-5,8-10}
{
  "$schema": "https://prioricode.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="prioricode.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Run `prioricode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/Prioritech-Indonesia-Optima/prioricode).
:::
...
```markdown title="~/.config/prioricode/agents/docs-writer.md"
---
...
```markdown title="~/.config/prioricode/agents/security-auditor.md"
---
