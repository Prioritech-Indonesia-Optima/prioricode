...
Det er to typer agenter i PrioriCode; primære agenter og underagenter.
...
PrioriCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
PrioriCode kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

PrioriCode kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `prioricode.json`:
...
- Per prosjekt: `.prioricode/agents/`
...
Hvis ingen temperatur er spesifisert, bruker PrioriCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale PrioriCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i PrioriCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [PrioriCode Zen](/docs/zen), vil du bruke `prioricode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `prioricode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/Prioritech-Indonesia-Optima/prioricode).
:::
