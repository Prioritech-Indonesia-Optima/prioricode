---
title: Tjener
description: Samhandle med PrioriCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `prioricode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en PrioriCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `prioricode.local` |
...
Sett `PRIORICODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `prioricode` som standard, eller sett `PRIORICODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `prioricode serve` og `prioricode web`.
...
Når du kjører `prioricode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk PrioriCode-serveren til å samhandle med PrioriCode programmatisk.
:::

Denne arkitekturen lar PrioriCode støtte flere klienter og lar deg samhandle med PrioriCode programmatisk.

Du kan kjøre `prioricode serve` for å starte en frittstående server. Hvis du har
PrioriCode TUI kjører, vil `prioricode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av PrioriCode [IDE](/docs/ide) plugins.
...
prioricode-serveren viser følgende APIer.
