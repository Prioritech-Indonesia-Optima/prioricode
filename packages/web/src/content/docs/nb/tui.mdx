---
title: TUI
description: Bruke prioricode-terminalbrukergrensesnittet.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

PrioriCode gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre PrioriCode starter TUI for gjeldende katalog.
...
Når du bruker PrioriCode TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til PrioriCode. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt PrioriCode. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `prioricode.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge PrioriCode venter etter leader key. Standard er `2000`.
...
Bruk `PRIORICODE_TUI_CONFIG` for å laste en egendefinert TUI-konfigurasjonsbane.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `prioricode.default`.
