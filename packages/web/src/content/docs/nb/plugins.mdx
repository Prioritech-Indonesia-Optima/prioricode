...
description: Skriv dine egne programtillegg for å utvide PrioriCode.
---

Plugins lar deg utvide PrioriCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til PrioriCode.
...
- `.prioricode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/prioricode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/prioricode/prioricode.json`)
2. Prosjektkonfigurasjon (`prioricode.json`)
3. Global plugin-katalog (`~/.config/prioricode/plugins/`)
4. Prosjektpluginkatalog (`.prioricode/plugins/`)
...
PrioriCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En PrioriCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide PrioriCode.
...
:::note
Hvis du bruker PrioriCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre PrioriCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til PrioriCode:
...
`tool`-hjelperen lager et tilpasset verktøy som PrioriCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for PrioriCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://prioricode.ai/docs/sdk) for detaljer.
