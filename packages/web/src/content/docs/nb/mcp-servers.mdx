...
Du kan legge til eksterne verktøy i PrioriCode ved å bruke _Model Context Protocol_ eller MCP. PrioriCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [PrioriCode Config](https://prioricode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="prioricode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/prioricode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="prioricode.jsonc" {15}
{
  "$schema": "https://prioricode.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
PrioriCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil PrioriCode:
...
Hvis serveren krever autentisering, vil PrioriCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `prioricode mcp auth <server-name>`.
...
```json title="prioricode.json" {7-11}
{
  "$schema": "https://prioricode.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
prioricode mcp auth my-oauth-server
```
...
```bash
prioricode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil PrioriCode lagre tokens sikkert i `~/.local/share/prioricode/mcp-auth.json`.
...
prioricode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i PrioriCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom PrioriCode-konfigurasjonen som alle andre verktøy.
...
```json title="prioricode.json" {11, 14-18}
{
  "$schema": "https://prioricode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="prioricode.json" {4-8}
{
  "$schema": "https://prioricode.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
prioricode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble PrioriCode til Sentry-kontoen din.
...
```json title="prioricode.json" {4-7}
{
  "$schema": "https://prioricode.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="prioricode.json" {7-9}
{
  "$schema": "https://prioricode.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver PrioriCode-samtale oppført som en separat økt.
