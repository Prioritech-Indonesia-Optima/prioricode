...
You can add external tools to PrioriCode using the _Model Context Protocol_, or MCP. PrioriCode supports both local and remote servers.
...
You can define MCP servers in your [PrioriCode Config](https://prioricode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/prioricode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
PrioriCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, PrioriCode will:
...
If the server requires authentication, PrioriCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `prioricode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, PrioriCode will store the tokens securely in `~/.local/share/prioricode/mcp-auth.json`.
...
Your MCPs are available as tools in PrioriCode, alongside built-in tools. So you can manage them through the PrioriCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect PrioriCode to your Sentry account.
