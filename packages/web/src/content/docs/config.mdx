---
title: Config
description: Using the PrioriCode JSON config.
---

You can configure PrioriCode using a JSON config file.

---

## Format

PrioriCode supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="prioricode.jsonc"
{
...
1. **Remote config** (from `.well-known/prioricode`) - organizational defaults
2. **Global config** (`~/.config/prioricode/prioricode.json`) - user preferences
3. **Custom config** (`PRIORICODE_CONFIG` env var) - custom overrides
4. **Project config** (`prioricode.json` in project) - project-specific settings
5. **`.prioricode` directories** - agents, commands, plugins
6. **Inline config** (`PRIORICODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/prioricode/` on macOS) - admin-controlled
...
:::note
The `.prioricode` and `~/.config/prioricode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/prioricode` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/prioricode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global PrioriCode config in `~/.config/prioricode/prioricode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/prioricode/tui.json`.
...
Add `prioricode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When PrioriCode starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `PRIORICODE_CONFIG` environment variable.

```bash
export PRIORICODE_CONFIG=/path/to/my/custom-config.json
prioricode run "Hello world"
```
...
Specify a custom config directory using the `PRIORICODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.prioricode` directory, and should
follow the same structure.

```bash
export PRIORICODE_CONFIG_DIR=/path/to/my/config-directory
prioricode run "Hello world"
```

The custom directory is loaded after the global config and `.prioricode` directories, so it **can override** their settings.
...
Drop an `prioricode.json` or `prioricode.jsonc` file in the system managed config directory:

| Platform | Path                                       |
| -------- | ------------------------------------------ |
| macOS    | `/Library/Application Support/prioricode/` |
| Linux    | `/etc/prioricode/`                         |
| Windows  | `%ProgramData%\prioricode`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, PrioriCode reads managed preferences from the `ai.prioricode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

PrioriCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.prioricode.managed.plist`
2. `/Library/Managed Preferences/ai.prioricode.managed.plist`

The plist keys map directly to `prioricode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.prioricode.managed` PayloadType. The PrioriCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.prioricode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.prioricode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.prioricode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`prioricode.ai/config.json`**](https://prioricode.ai/config.json).

TUI config uses [**`prioricode.ai/tui.json`**](https://prioricode.ai/tui.json).
...
Use `PRIORICODE_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `prioricode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `prioricode serve` and `prioricode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your PrioriCode server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `prioricode.local`. Useful for running multiple instances on the same network.
...
If not specified, PrioriCode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your PrioriCode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, PrioriCode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny PrioriCode actions on configured resources. Currently, policies can control which providers PrioriCode may use.
...
PrioriCode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, PrioriCode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/prioricode/agents/` or `.prioricode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, PrioriCode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`prioricode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/prioricode/commands/` or `.prioricode/commands/`. [Learn more here](/docs/commands).
...
PrioriCode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
PrioriCode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, prioricode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend PrioriCode with custom tools, hooks, and integrations.

Place plugin files in `.prioricode/plugins/` or `~/.config/prioricode/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="prioricode.json"
{
  "$schema": "https://prioricode.ai/config.json",
  "plugin": ["prioricode-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict PrioriCode to only use specific providers rather than disabling them one by one.
