...
With PrioriCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, PrioriCode uses our own `prioricode` theme.
...
PrioriCode comes with several built-in themes.
...
- Want PrioriCode to match their terminal's appearance
...
PrioriCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/prioricode/themes/*.json` or `$XDG_CONFIG_HOME/prioricode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.prioricode/themes/*.json`
4. **Current working directory** - Defined in `./.prioricode/themes/*.json`
