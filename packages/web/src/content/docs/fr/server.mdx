---
title: Serveur
description: Interagissez avec le serveur prioricode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `prioricode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client prioricode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `prioricode.local` |
...
Définissez `PRIORICODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `prioricode`, ou définissez `PRIORICODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `prioricode serve` et à `prioricode web`.
...
Lorsque vous exécutez `prioricode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur prioricode pour interagir avec prioricode par programmation.
:::

Cette architecture permet à prioricode de prendre en charge plusieurs clients et vous permet d'interagir avec prioricode par programme.

Vous pouvez exécuter `prioricode serve` pour démarrer un serveur autonome. Si vous avez le
prioricode TUI en cours d'exécution, `prioricode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins PrioriCode [IDE](/docs/ide).
...
Le serveur prioricode expose les API suivantes.
