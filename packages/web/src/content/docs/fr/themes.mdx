...
Avec PrioriCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, PrioriCode utilise notre propre thème `prioricode`.
...
PrioriCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que PrioriCode corresponde à l'apparence de leur terminal
...
PrioriCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/prioricode/themes/*.json` ou `$XDG_CONFIG_HOME/prioricode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.prioricode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.prioricode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/prioricode/themes
vim ~/.config/prioricode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .prioricode/themes
vim .prioricode/themes/my-theme.json
```
...
{
  "$schema": "https://prioricode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
