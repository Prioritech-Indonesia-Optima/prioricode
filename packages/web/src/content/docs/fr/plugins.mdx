---
title: Plugins
description: Écrivez vos propres plugins pour étendre PrioriCode.
---

Les plugins vous permettent d'étendre PrioriCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de PrioriCode.
...
- `.prioricode/plugins/` - Plugins au niveau du projet
- `~/.config/prioricode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/prioricode/node_modules/`.
...
1. Configuration globale (`~/.config/prioricode/prioricode.json`)
2. Configuration du projet (`prioricode.json`)
3. Répertoire global des plugins (`~/.config/prioricode/plugins/`)
4. Répertoire des plugins du projet (`.prioricode/plugins/`)
...
PrioriCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK prioricode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre prioricode.
...
:::note
Si vous utilisez l'application de bureau PrioriCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez prioricode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à prioricode :
...
L'assistant `tool` crée un outil personnalisé que prioricode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour prioricode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://prioricode.ai/docs/sdk) pour plus de détails.
