---
title: Servidor
description: Interactuar con el servidor prioricode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `prioricode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente prioricode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `prioricode.local` |
...
Configure `PRIORICODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `prioricode`, o configure `PRIORICODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `prioricode serve` como a `prioricode web`.
...
Cuando ejecuta `prioricode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor prioricode para interactuar con prioricode mediante programación.
:::

Esta arquitectura permite que prioricode admita múltiples clientes y le permite interactuar con prioricode mediante programación.

Puede ejecutar `prioricode serve` para iniciar un servidor independiente. Si tienes el
prioricode TUI ejecutándose, `prioricode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos PrioriCode [IDE](/docs/ide).
...
El servidor prioricode expone las siguientes API.
