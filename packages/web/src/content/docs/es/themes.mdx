...
Con PrioriCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, PrioriCode usa nuestro propio tema `prioricode`.
...
PrioriCode viene con varios temas integrados.
...
- Quiere que PrioriCode coincida con la apariencia de su terminal
...
PrioriCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/prioricode/themes/*.json` o `$XDG_CONFIG_HOME/prioricode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.prioricode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.prioricode/themes/*.json`
