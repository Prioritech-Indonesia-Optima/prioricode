...
Las habilidades del agente permiten a PrioriCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
PrioriCode busca estas ubicaciones:

- Configuración del proyecto: `.prioricode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/prioricode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, PrioriCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.prioricode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/prioricode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.prioricode/skills/git-release/SKILL.md` así:
...
PrioriCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `prioricode.json`:
...
**Para agentes integrados** (en `prioricode.json`):
