...
description: Cree herramientas que LLM pueda llamar en prioricode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de prioricode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.prioricode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/prioricode/tools/`.
...
```ts title=".prioricode/tools/database.ts" {1}
...
```ts title=".prioricode/tools/math.ts"
import { tool } from "@prioricode/plugin"
...
```ts title=".prioricode/tools/bash.ts"
import { tool } from "@prioricode/plugin"
...
```ts title=".prioricode/tools/project.ts" {8}
import { tool } from "@prioricode/plugin"
...
```python title=".prioricode/tools/add.py"
import sys
...
```ts title=".prioricode/tools/python-add.ts" {10}
import { tool } from "@prioricode/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".prioricode/tools/add.py")
