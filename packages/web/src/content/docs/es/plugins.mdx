---
title: Complementos
description: Escriba sus propios complementos para extender PrioriCode.
---

Los complementos le permiten extender PrioriCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de PrioriCode.
...
- `.prioricode/plugins/` - Complementos a nivel de proyecto
- `~/.config/prioricode/plugins/` - Complementos globales
...
```json title="prioricode.json"
{
  ...
  "plugin": ["prioricode-helicone-session", "prioricode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/prioricode/node_modules/`.
...
1. Configuración global (`~/.config/prioricode/prioricode.json`)
2. Configuración del proyecto (`prioricode.json`)
3. Directorio global de complementos (`~/.config/prioricode/plugins/`)
4. Directorio de complementos del proyecto (`.prioricode/plugins/`)
...
```json title=".prioricode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
PrioriCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".prioricode/plugins/my-plugin.ts"
...
```js title=".prioricode/plugins/example.js"
...
- `client`: Un cliente SDK prioricode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar prioricode.
...
```js title=".prioricode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "prioricode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio PrioriCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que prioricode lea archivos `.env`:

```javascript title=".prioricode/plugins/env-protection.js"
...
```javascript title=".prioricode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a prioricode:

```ts title=".prioricode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@prioricode/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que prioricode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para prioricode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://prioricode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".prioricode/plugins/compaction.ts"
import type { Plugin } from "@prioricode/plugin"
...
```ts title=".prioricode/plugins/custom-compaction.ts"
import type { Plugin } from "@prioricode/plugin"
