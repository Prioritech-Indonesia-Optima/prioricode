...
Las herramientas permiten que LLM realice acciones en su código base. PrioriCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en PrioriCode.
...
:::note
Esta herramienta solo está disponible cuando `PRIORICODE_EXPERIMENTAL_LSP_TOOL=true` (o `PRIORICODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor PrioriCode o cuando la variable de entorno `PRIORICODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar PrioriCode:

```bash
PRIORICODE_ENABLE_EXA=1 prioricode
```
