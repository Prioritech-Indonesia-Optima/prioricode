---
title: Eklentiler
description: PrioriCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek PrioriCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya PrioriCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.prioricode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/prioricode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/prioricode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/prioricode/prioricode.json`)
2. Project config (`prioricode.json`)
3. Global eklenti dizini (`~/.config/prioricode/plugins/`)
4. Proje eklenti dizini (`.prioricode/plugins/`)
...
PrioriCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir PrioriCode SDK istemcisi.
...
PrioriCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
PrioriCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
PrioriCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca PrioriCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, PrioriCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra PrioriCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://prioricode.ai/docs/sdk) bakın.
