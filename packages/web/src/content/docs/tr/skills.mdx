...
Ajan becerileri, prioricode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
prioricode su konumlari tarar:

- Proje konfigurasyonu: `.prioricode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/prioricode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda prioricode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.prioricode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/prioricode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.prioricode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
prioricode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`prioricode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`prioricode.json` icinde):
