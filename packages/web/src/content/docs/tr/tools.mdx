...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. prioricode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
prioricode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `PRIORICODE_EXPERIMENTAL_LSP_TOOL=true` (veya `PRIORICODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca prioricode provider kullanılırken veya `PRIORICODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

prioricode başlatırken etkinleştirmek için:
