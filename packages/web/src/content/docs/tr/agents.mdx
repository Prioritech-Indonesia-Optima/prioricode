...
prioricode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
prioricode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
PrioriCode, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

PrioriCode iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `prioricode.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.prioricode/agents/`
...
Sıcaklık belirtilmezse prioricode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global prioricode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
prioricode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [PrioriCode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `prioricode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `prioricode models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/Prioritech-Indonesia-Optima/prioricode).
:::
