...
_Model Bağlam Protokolü_ veya MCP kullanarak prioricode'a harici araçlar ekleyebilirsiniz. prioricode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [prioricode Config](https://prioricode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/prioricode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
prioricode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde prioricode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa prioricode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `prioricode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından prioricode, belirteçleri `~/.local/share/prioricode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra prioricode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi prioricode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve prioricode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
