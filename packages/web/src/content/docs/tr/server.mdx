---
title: Sunucu
description: prioricode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`prioricode serve` komutu, prioricode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `prioricode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `PRIORICODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `prioricode` değeridir; değiştirmek isterseniz `PRIORICODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `prioricode serve` hem de `prioricode web` için geçerlidir.
...
`prioricode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
prioricode ile programatik etkileşim için prioricode sunucusunu kullanın.
:::

Bu mimari, prioricode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`prioricode serve` ile bağımsız bir sunucu başlatabilirsiniz. prioricode TUI açıksa, `prioricode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum prioricode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
prioricode sunucusu aşağıdaki API'leri sunar.
