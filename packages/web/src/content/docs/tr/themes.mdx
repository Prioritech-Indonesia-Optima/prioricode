...
prioricode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak prioricode kendi `prioricode` temasını kullanır.
...
prioricode birden fazla yerleşik temayla gelir.
...
- prioricode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
prioricode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/prioricode/themes/*.json` veya `$XDG_CONFIG_HOME/prioricode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.prioricode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.prioricode/themes/*.json`
