...
description: prioricode dile özgü biçimlendiriciler kullanır.
---

prioricode, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

prioricode, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, prioricode bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

prioricode bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri prioricode yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
