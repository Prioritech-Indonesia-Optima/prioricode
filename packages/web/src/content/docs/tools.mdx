...
Tools allow the LLM to perform actions in your codebase. PrioriCode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in PrioriCode.
...
:::note
This tool is only available when `PRIORICODE_EXPERIMENTAL_LSP_TOOL=true` (or `PRIORICODE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the PrioriCode or PrioriCode Go provider, or when either the `PRIORICODE_ENABLE_EXA` or `PRIORICODE_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching PrioriCode:

```bash
PRIORICODE_ENABLE_EXA=1 prioricode
# or
PRIORICODE_ENABLE_PARALLEL=1 prioricode
```
