---
title: LSP Servers
description: PrioriCode integrates with your LSP servers.
---

PrioriCode can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

PrioriCode comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `PRIORICODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and prioricode opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your prioricode config.
...
```json title="prioricode.json" {5-8}
{
  "$schema": "https://prioricode.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="prioricode.json" {5-13}
{
  "$schema": "https://prioricode.ai/config.json",
  "lsp": {
    "custom-lsp": {
