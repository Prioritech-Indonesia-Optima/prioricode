---
title: الخادم
description: تفاعل مع خادم prioricode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `prioricode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل prioricode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `prioricode.local` |
...
عيّن `PRIORICODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `prioricode`، أو عيّن `PRIORICODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `prioricode serve` و `prioricode web`.
...
عند تشغيل `prioricode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم prioricode للتفاعل مع prioricode برمجيا.
:::

تتيح هذه البنية لـ prioricode دعم عدة عملاء وتمكّنك من التفاعل مع prioricode برمجيا.

يمكنك تشغيل `prioricode serve` لبدء خادم مستقل. إذا كانت واجهة prioricode في terminal (TUI)
قيد التشغيل، فسيبدأ `prioricode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات PrioriCode لـ [IDE](/docs/ide).
...
يعرض خادم prioricode واجهات API التالية.
