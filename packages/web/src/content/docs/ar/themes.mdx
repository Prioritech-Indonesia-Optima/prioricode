...
مع PrioriCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم PrioriCode سمتنا `prioricode`.
...
يأتي PrioriCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق PrioriCode مظهر terminal لديهم
...
يدعم PrioriCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/prioricode/themes/*.json` أو `$XDG_CONFIG_HOME/prioricode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.prioricode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.prioricode/themes/*.json`
