---
title: المزوّدون
description: استخدام أي موفّر LLM في PrioriCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم PrioriCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات PrioriCode.
...
في `~/.local/share/prioricode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات PrioriCode.
...
PrioriCode Zen هي قائمة نماذج يوفّرها فريق PrioriCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع PrioriCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع PrioriCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر prioricode، ثم انتقل إلى [prioricode.ai/auth](https://prioricode.ai/auth).
...
يعمل مثل أي مزوّد آخر في PrioriCode واستخدامه اختياري بالكامل.

---

## PrioriCode Go

PrioriCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق PrioriCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع PrioriCode.

1. شغّل الأمر `/connect` في TUI، واختر `PrioriCode Go`، ثم انتقل إلى [prioricode.ai/auth](https://prioricode.ai/zen).
...
لاستخدام Amazon Bedrock مع PrioriCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل prioricode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `prioricode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل PrioriCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين prioricode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل prioricode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات prioricode.
...
تخزين الرمز في مساحة تخزين مصادقة prioricode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم PrioriCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر PrioriCode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`prioricode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع prioricode:
...
لاستخدام Google Vertex AI مع PrioriCode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر prioricode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [prioricode-helicone-session](https://github.com/H2Shami/prioricode-helicone-session) لتسجيل كل محادثة PrioriCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة PrioriCode مدرجة كجلسة مستقلة.
...
يمكنك ضبط prioricode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط prioricode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط prioricode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع PrioriCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/prioricode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع PrioriCode:
...
3. انسخ مفتاح API لاستخدامه في PrioriCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في PrioriCode، يجب سحب معلومات النموذج محليا:
...
PrioriCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق PrioriCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>PrioriCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **PrioriCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ PrioriCode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات prioricode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات prioricode. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Prioricode:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات prioricode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع prioricode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `prioricode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ PrioriCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `prioricode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات prioricode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات prioricode.
