---
title: Konfiguracija
description: Korištenje PrioriCode JSON konfiguracije.
---

Možete konfigurirati PrioriCode koristeći JSON konfiguracijski fajl.

---

## Format

PrioriCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/prioricode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/prioricode/prioricode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`PRIORICODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`prioricode.json` u projektu) - postavke specifične za projekat
5. **`.prioricode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`PRIORICODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.prioricode` i `~/.config/prioricode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/prioricode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu PrioriCode konfiguraciju u `~/.config/prioricode/prioricode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/prioricode/tui.json`.
...
Dodajte `prioricode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se PrioriCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `PRIORICODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `PRIORICODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.prioricode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.prioricode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`prioricode.ai/config.json`**](https://prioricode.ai/config.json).

TUI konfiguracija koristi [**`prioricode.ai/tui.json`**](https://prioricode.ai/tui.json).
...
Koristite `PRIORICODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `prioricode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `prioricode serve` i `prioricode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš PrioriCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `prioricode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj PrioriCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, PrioriCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/prioricode/agents/` ili `.prioricode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, PrioriCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`prioricode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/prioricode/commands/` ili `.prioricode/commands/`. [Saznajte više ovdje](/docs/commands).
...
PrioriCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, PrioriCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju PrioriCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.prioricode/plugins/` ili `~/.config/prioricode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite PrioriCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
