...
PrioriCode Go je povoljna pretplata od **$10/mjesečno** koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u PrioriCode-u. Pretplatite se na PrioriCode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili PrioriCode.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa PrioriCode-om.
:::
...
PrioriCode Go vam daje pristup ovim modelima za **$10/mjesečno**.

---

## Kako funkcioniše

PrioriCode Go radi kao bilo koji drugi provajder u PrioriCode-u.

1. Prijavite se na **<a href={console}>PrioriCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `PrioriCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na PrioriCode Go.
:::
...
PrioriCode Go je osmišljen za [PrioriCode](https://prioricode.ai) i druge agente za programiranje
...
3. Slati stabilan ID sesije u zaglavlju `x-prioricode-session` za svaki razgovor kako bismo mogli optimizovati usmjeravanje i
   keširanje promptova.

### Provjereni klijenti

Pored PrioriCode-a, potvrđeno je da sljedeći klijenti ispravno rade
s PrioriCode Go. Ipak, ne garantiramo da će nastaviti raditi i u budućnosti.

| Klijent           | Podrška za sesije                                                                                                                                                                                                                                            |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Hermes**        | Buildovi koji sadrže [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) šalju zaglavlje u glavnim i pomoćnim PrioriCode zahtjevima. Ispravka je spojena nakon verzije v0.21.0; samo to izdanje je ne sadrži.                             |
...
| **ZCode**         | Go prepoznaje njegovo izvorno zaglavlje sesije. Naš [zahtjev za `x-prioricode-session`](https://github.com/zai-org/feedback/issues/492) i dalje je otvoren, ali slanje tog konkretnog zaglavlja više nije potrebno.                                          |
| **Pi**            | Trenutni buildovi šalju informacije o sesiji za PrioriCode. Ažurirajte starije instalacije.                                                                                                                                                                  |
...
| **Kilo Code CLI** | Buildovi koji sadrže [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) vraćaju PrioriCode zaglavlja sesije. Ova ispravka obuhvata CLI, ali ne i VS Code ekstenziju. Pogledajte [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
[Model id](/docs/config/#models) u vašoj PrioriCode konfiguraciji
koristi format `prioricode-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`prioricode-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo PrioriCode Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz PrioriCode koristite i bilo kojeg drugog provajdera.
