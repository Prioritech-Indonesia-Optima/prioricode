...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. PrioriCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u PrioriCode.
...
:::note
Ovaj alat je dostupan samo kada je `PRIORICODE_EXPERIMENTAL_LSP_TOOL=true` (ili `PRIORICODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz PrioriCode provajdera ili kada je varijabla `PRIORICODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju PrioriCode:
