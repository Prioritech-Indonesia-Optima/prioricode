...
Možete dodati vanjske alate u PrioriCode koristeći _Model Context Protocol_, ili MCP. PrioriCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [PrioriCode Config](https://prioricode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/prioricode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
PrioriCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, PrioriCode će:
...
Ako server zahtijeva autentifikaciju, PrioriCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `prioricode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, PrioriCode sigurno čuva tokene u `~/.local/share/prioricode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u PrioriCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz PrioriCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete PrioriCode sa vašim Sentry nalogom.
