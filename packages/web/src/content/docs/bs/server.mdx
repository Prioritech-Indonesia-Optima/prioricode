---
title: Server
description: Komunicirajte s prioricode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `prioricode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji prioricode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `prioricode.local` |
...
Postavite `PRIORICODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `prioricode`, ili postavite `PRIORICODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `prioricode serve` i za `prioricode web`.
...
Kada pokrenete `prioricode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite prioricode server za programsku interakciju sa prioricode-om.
:::

Ova arhitektura omogucava prioricode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `prioricode serve` da startate standalone server. Ako je prioricode TUI vec pokrenut, `prioricode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste PrioriCode [IDE](/docs/ide) pluginovi.
...
prioricode server izlaže sljedece API-je.
