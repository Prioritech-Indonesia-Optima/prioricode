...
Da biste otklonili probleme s PrioriCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\prioricode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `prioricode --log-level DEBUG`.

---

## Pohrana

PrioriCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/prioricode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\prioricode`
...
PrioriCode Desktop pokreće lokalni PrioriCode server (`prioricode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `PrioriCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/prioricode/prioricode.jsonc` (ili `~/.config/prioricode/prioricode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/prioricode/prioricode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\prioricode\prioricode.jsonc`
...
PrioriCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/prioricode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\prioricode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga PrioriCode mogao ponovo izgraditi.

1. Potpuno zatvorite PrioriCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/prioricode`
- **Linux**: obrišite `~/.cache/prioricode` (ili pokrenite `rm -rf ~/.cache/prioricode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\prioricode`

3. Ponovo pokrenite PrioriCode Desktop.

---

### Rješavanje problema sa vezom na serveru

PrioriCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `prioricode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `PRIORICODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `PRIORICODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, PrioriCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa PrioriCode karakteristikama.

---

### Obavještenja se ne prikazuju

PrioriCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za PrioriCode u postavkama vašeg OS-a, i
...
1. Zatvorite PrioriCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka PrioriCode Desktop aplikacije):

- `prioricode.settings.dat` (URL zadanog servera za desktop)
- `prioricode.global.dat` i `prioricode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s PrioriCode:
...
Da saznate kojim modelima imate pristup, pokrenite `prioricode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\prioricode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. PrioriCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\prioricode`

2. Ponovo pokrenite PrioriCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti PrioriCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
PrioriCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
