...
Agent skills omogucavaju PrioriCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
PrioriCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.prioricode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/prioricode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, PrioriCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.prioricode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/prioricode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.prioricode/skills/git-release/SKILL.md` ovako:
...
PrioriCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `prioricode.json`:
...
**Za ugradene agente** (u `prioricode.json`):
