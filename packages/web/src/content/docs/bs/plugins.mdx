---
title: Dodaci
description: Napišite vlastite dodatke za proširenje PrioriCode.
---

Dodaci vam omogućavaju da proširite PrioriCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja PrioriCode.
...
- `.prioricode/plugins/` - Dodaci na nivou projekta
- `~/.config/prioricode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/prioricode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/prioricode/prioricode.json`)
2. Konfiguracija projekta (`prioricode.json`)
3. Globalni direktorij dodataka (`~/.config/prioricode/plugins/`)
4. Direktorij dodataka projekta (`.prioricode/plugins/`)
...
PrioriCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Prioricode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje PrioriCode.
...
Spriječite prioricode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u prioricode:
...
Pomoćnik `tool` kreira prilagođeni alat koji prioricode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za prioricode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://prioricode.ai/docs/sdk) za detalje.
