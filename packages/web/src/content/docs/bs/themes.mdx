...
U PrioriCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, PrioriCode koristi nasu `prioricode` temu.
...
PrioriCode dolazi sa vise ugradenih tema.
...
- Zele da PrioriCode odgovara izgledu njihovog terminala
...
PrioriCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/prioricode/themes/*.json` ili `$XDG_CONFIG_HOME/prioricode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.prioricode/themes/*.json`
4. **Trenutni radni direktorij** - `./.prioricode/themes/*.json`
