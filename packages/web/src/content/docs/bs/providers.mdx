---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u PrioriCode.
---

import config from "../../../../config.mjs"
export const console = config.console

PrioriCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj PrioriCode konfiguraciji.
...
u `~/.local/share/prioricode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem PrioriCode
config.
...
PrioriCode Zen je lista modela koje je obezbedio PrioriCode tim koji su bili
testirano i potvrđeno da dobro radi sa PrioriCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa PrioriCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite prioricode i idite na [prioricode.ai/auth](https://prioricode.ai/auth).
...
Radi kao i svaki drugi provajder u PrioriCode i potpuno je opcionalan za korištenje.

---

## PrioriCode Go

PrioriCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža PrioriCode tim i koji su testirani i verificirani da dobro rade s PrioriCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `PrioriCode Go` i idite na [prioricode.ai/auth](https://prioricode.ai/zen).
...
Da biste koristili Amazon Bedrock s PrioriCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete prioricode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `prioricode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u PrioriCode.
:::
...
Možete konfigurirati prioricode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi prioricode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju prioricode konfiguraciju.
...
da pohrani token u prioricode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
PrioriCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate PrioriCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`prioricode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `prioricode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s prioricode:
...
Za korištenje Google Vertex AI s PrioriCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće prioricode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem prioricodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [prioricode-helicone-session](https://github.com/H2Shami/prioricode-helicone-session) da automatski prijavite svaki PrioriCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki PrioriCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati prioricode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati prioricode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati prioricode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za PrioriCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/prioricode) za detalje.
:::
...
Da biste koristili Ollama Cloud s PrioriCode:
...
3. Kopirajte API ključ za korištenje u PrioriCode.
...
6. **Važno**: Prije upotrebe modela oblaka u PrioriCode, morate lokalno povući informacije o modelu:
...
PrioriCode Zen je lista testiranih i verifikovanih modela koju je obezbedio PrioriCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>PrioriCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **PrioriCode Zen**.

```txt
   /connect
```

3. Unesite svoj PrioriCode API ključ.
...
Također možete dodati dodatne modele putem vaše prioricode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše prioricode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Prioricodeom:
...
Također možete prilagoditi modele kroz svoju prioricode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s prioricode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `prioricode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju PrioriCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `prioricode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite PrioriCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj prioricode konfiguraciji.
