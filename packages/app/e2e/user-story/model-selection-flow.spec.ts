...
const directory = "C:/PrioriCode/NewProject"

test("creates a session in a new project, connects PrioriCode Go, and selects its model", async ({ page }) => {
  ...
  await mockPrioriCodeServer(page, {
    ...
    provider: () => ({
      ...
      connected: connectedGo ? ["prioricode", "prioricode-go"] : ["prioricode"],
      default: { providerID: "prioricode", modelID: "free-model" },
    }),
    integrationMethods: { "prioricode-go": [{ type: "api", label: "API key" }] },
    onConnectKey: (input) => {
      connections.push(input)
      if (input.integrationID === "prioricode-go") pendingGo = true
    },
    onInstanceDispose: () => {
  ...
  await expect(page.locator('[data-section="free-models"]')).toContainText("Free models provided by PrioriCode")

  await page.locator('[data-provider-id="prioricode-go"]').click()
  ...
  expect(connections).toEqual([{ integrationID: "prioricode-go", body: { type: "api", key: "mock-go-api-key" } }])

  await expect(modelControl).toHaveAttribute("data-control-type", "popover")
  await modelControl.click()
  const goModel = page.locator('[data-option-key="prioricode-go:go-model-1"]')
