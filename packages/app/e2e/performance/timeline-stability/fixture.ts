...
import { SessionStatusEvent } from "@prioricode/schema/session-status-event"
import { SessionV1 } from "@prioricode/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/PrioriCode/TimelineStability"
...
export const model = { providerID: "prioricode", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("prioricode.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    default: { providerID: "prioricode", modelID: "claude-opus-4-6" },
  }
}
