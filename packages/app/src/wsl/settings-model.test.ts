...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when PrioriCode needs attention", () => {
    expect(wslPrioricodeAction(undefined)).toBeUndefined()
    expect(
      wslPrioricodeAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("wsl.onboarding.installPrioricode")
    expect(
      wslPrioricodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/prioricode",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("wsl.onboarding.updatePrioricode")
    expect(
      wslPrioricodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/prioricode",
  ...
  test("does not report ready when PrioriCode is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        prioricodeChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.prioricode/bin/prioricode",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "prioricode is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installPrioricode" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-prioricode")
  })
