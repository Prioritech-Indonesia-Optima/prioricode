...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-prioricode" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslPrioricodeAction(check?: WslPrioricodeCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installPrioricode"
  if (check.matchesDesktop === false) return "wsl.onboarding.updatePrioricode"
}
...
export function addServerViewModel(input: {
  ...
  const prioricodeCheck = selectedDistro ? (state?.prioricodeChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.prioricodeChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updatePrioricode" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.prioricodeMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installPrioricode" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  prioricodeCheck: WslPrioricodeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const probingPrioricode =
    probingSelected ||
    (ready &&
      (!input.prioricodeCheck ||
        (!!input.selectedDistro &&
          input.state?.job?.kind === "probe-addable" &&
          input.state.job.distros.includes(input.selectedDistro))))
  const installingPrioricode =
    input.state?.job?.kind === "install-prioricode" && input.state.job.distro === input.selectedDistro
  if (!ready || probingPrioricode) {
    return {
      variant: "contrast",
  ...
  if (!addServerPrioricodeReady(input.prioricodeCheck)) {
    const update = !!input.prioricodeCheck?.resolvedPath && input.prioricodeCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingPrioricode
        ? { key: "wsl.onboarding.updatingPrioricode" }
        : update
          ? { key: "wsl.onboarding.updatePrioricode" }
          : { key: "wsl.onboarding.installPrioricode" },
      disabled: !!input.state?.job || input.adding,
      action: "install-prioricode",
      loading: installingPrioricode,
...
function addServerPrioricodeReady(check: WslPrioricodeCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.prioricodeChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.prioricodeChecks[item.name]) return [`prioricode:${item.name}`]
    return []
  })
