...
import { type Session } from "@prioricode/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  compareSessionTime,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("prioricode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("prioricode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("prioricode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("prioricode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("prioricode://open-project")).toBeUndefined()
    expect(parseDeepLink("prioricode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "prioricode://open-project?directory=/a",
      "prioricode://other?directory=/b",
      "prioricode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("prioricode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("prioricode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("prioricode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("prioricode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "prioricode://new-session?directory=/a",
      "prioricode://open-project?directory=/b",
      "prioricode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __PRIORICODE__: {
        deepLinks: ["prioricode://open-project?directory=/a"],
      },
    } as unknown as Window & { __PRIORICODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["prioricode://open-project?directory=/a"])
