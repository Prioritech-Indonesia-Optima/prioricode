...
import { Accordion } from "@prioricode/ui/accordion"
import { Button } from "@prioricode/ui/button"
import { Card } from "@prioricode/ui/card"
import {
  ContextToolGroup,
  Message,
  MessageDivider,
  ...
  partDefaultOpen,
  type UserActions,
} from "@prioricode/session-ui/message-part"
import { DiffChanges } from "@prioricode/ui/diff-changes"
import { FileIcon } from "@prioricode/ui/file-icon"
import { Icon } from "@prioricode/ui/icon"
import { IconButton } from "@prioricode/ui/icon-button"
import { Icon as IconV2 } from "@prioricode/ui/v2/icon"
import { IconButtonV2 } from "@prioricode/ui/v2/icon-button-v2"
import { DropdownMenu } from "@prioricode/ui/dropdown-menu"
import { MenuV2 } from "@prioricode/ui/v2/menu-v2"
import { Dialog } from "@prioricode/ui/dialog"
import { DialogFooter, DialogHeader, DialogTitleGroup, DialogV2 } from "@prioricode/ui/v2/dialog-v2"
import { InlineInput } from "@prioricode/ui/inline-input"
import { ButtonV2 } from "@prioricode/ui/v2/button-v2"
import { SessionRetry } from "@prioricode/session-ui/session-retry"
import { isScrollKeyTarget, scrollKey, scrollKeyOwner, ScrollView } from "@prioricode/ui/scroll-view"
import { StickyAccordionHeader } from "@prioricode/ui/sticky-accordion-header"
import { TextField } from "@prioricode/ui/text-field"
import { TextReveal } from "@prioricode/ui/text-reveal"
import { TextShimmer } from "@prioricode/ui/text-shimmer"
import type {
  AssistantMessage,
...
import { getDirectory, getFilename } from "@prioricode/core/util/path"
import { Popover as KobaltePopover } from "@kobalte/core/popover"
import { normalize } from "@prioricode/session-ui/session-diff"
import { useFileComponent } from "@prioricode/ui/context/file"
      ...
      >
        ...
        >
          <button
            type="button"
            aria-label={language.t("session.messages.jumpToLatest")}
            class="pointer-events-auto flex items-center justify-center w-8 h-7 px-2 py-1.5 rounded-lg border-none cursor-pointer text-pc-text-base backdrop-blur-[2px]"
            style={{
              background: "color-mix(in srgb, var(--pc-bg-base) 92%, transparent)",
              "box-shadow": "var(--pc-elevation-raised), 0px 2px 8px var(--pc-bg-base)",
            }}
            onClick={props.onResumeScroll}
          >
                ...
                <div class="flex items-center min-w-0 flex-1 w-full">
                  <Show when={parentID()}>
                    <button
                      ...
                      class="min-w-0 max-w-[40%] truncate pl-2 text-[13px] font-[530] leading-4 tracking-[-0.04px] text-pc-text-faint transition-colors hover:text-pc-text-muted"
                    ...
                    <span
                      ...
                      class="-translate-y-[0.5px] pl-2 pr-1 text-[11px] font-medium text-pc-text-faint"
                      aria-hidden="true"
                    >
                      /
                    </span>
                  ...
                  <Show when={childTitle() || title.editing}>
                    <Show
                      ...
                      fallback={
                        <h1
                          ...
                          classList={{
                            "truncate text-[13px] font-[530] leading-4 tracking-[-0.04px] text-pc-text-base": true,
                            "w-fit rounded-[6px] px-2 py-1 hover:bg-pc-overlay-hover":
                              settings.general.newLayoutDesigns(),
                    ...
                    >
                      <InlineInput
                        ...
                        classList={{
                          "block text-[13px] font-[530] leading-4 tracking-[-0.04px] text-pc-text-base": true,
                      ...
                      >
                        <KobaltePopover.Portal>
                          <KobaltePopover.Content
                            ...
                            classList={{
                              "flex w-80 max-w-none flex-col items-start gap-3 rounded-[12px] border-0 bg-pc-bg-layer-01 p-3 shadow-[var(--pc-elevation-floating)]":
                                settings.general.newLayoutDesigns(),
                            }}
                          ...
                          >
                            ...
                            >
                              <div class="flex w-full flex-col gap-1.5 px-0.5 pt-0.5">
                                <div class="select-none text-[13px] font-[530] leading-none tracking-[-0.04px] text-pc-text-base [font-variation-settings:'slnt'_0]">
                                  {language.t("session.share.popover.title")}
                                </div>
                                <div class="select-none text-[13px] font-[440] leading-5 tracking-[-0.04px] text-pc-text-muted [font-variation-settings:'slnt'_0]">
                                  {shareUrl()
                                    ? language.t("session.share.popover.description.shared")
                                    : language.t("session.share.popover.description.unshared")}
                                </div>
                              ...
                              <div class="flex w-full flex-col gap-2">
                                ...
                                >
                                  <div class="flex flex-col gap-2">
                                    <div
                                      class="flex h-8 w-full items-center gap-1.5 rounded-[6px] py-1 pl-2.5 pr-1.5 shadow-[var(--pc-elevation-button-neutral)]"
                                      style={{
                                        background:
                                          "linear-gradient(180deg, var(--pc-alpha-light-2) 0%, var(--pc-alpha-light-0) 100%), var(--pc-bg-button-neutral)",
                                      }}
                                    >
                                      <div
                                        class="min-w-0 flex-1 truncate select-text cursor-text text-[13px] font-[440] leading-5 tracking-[-0.04px] text-pc-text-base [font-variation-settings:'slnt'_0]"
