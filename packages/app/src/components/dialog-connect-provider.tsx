import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@prioricode/client/promise"
...
import { List, type ListRef } from "@prioricode/ui/list"
import { ProviderIcon } from "@prioricode/ui/provider-icon"
import { Spinner } from "@prioricode/ui/spinner"
import { Tag } from "@prioricode/ui/tag"
import { TextField } from "@prioricode/ui/text-field"
import { ButtonV2 } from "@prioricode/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@prioricode/ui/v2/dialog-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "prioricode-go") return language.t("dialog.provider.prioricodeGo.tagline")
    return undefined
  }
...
function ProviderPickerV2(props: {
  ...
  const featured = ["prioricode", "prioricode-go", "anthropic", "openai", "google", "openrouter", "vercel"]
    ...
    if (method.type === "oauth") {
      ...
      await serverSDK()
        ...
        .then((x) => {
          ...
          if (props.provider === "prioricode" && platform.platform === "desktop") {
            const url = new URL(x.data.url)
            url.searchParams.set("client_id", "prioricode-desktop")
            x.data.url = url.href
          }
  ...
  function ApiAuthView() {
        ...
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-pc-text-muted">
          ...
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.prioricodeZen.line1")}</div>
              <div>{language.t("provider.connect.prioricodeZen.line2")}</div>
              <div>
                ...
                {language.t("provider.connect.prioricodeZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "prioricode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.prioricodeZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.prioricodeZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.prioricodeZen.visit.prefix")}
                <ExternalLink href="https://prioricode.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.prioricodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.prioricodeZen.visit.suffix")}
              </div>
