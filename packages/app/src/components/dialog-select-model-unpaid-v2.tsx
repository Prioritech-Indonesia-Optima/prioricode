...
import { useTheme } from "@prioricode/ui/theme"
...
const featuredProviders = ["prioricode", "prioricode-go", "openai", "anthropic", "google", "github-copilot"]
...
export const DialogSelectModelUnpaidV2: Component<{ model?: ModelState }> = (props) => {
  ...
  const isFree = (item: ReturnType<ModelState["list"]>[number]) =>
    item.provider.id === "prioricode" && (!item.cost || item.cost.input === 0)
