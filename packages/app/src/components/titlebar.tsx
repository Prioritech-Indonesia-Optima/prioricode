...
import { Tooltip, TooltipKeybind } from "@prioricode/ui/tooltip"
import { IconButtonV2 } from "@prioricode/ui/v2/icon-button-v2"
import { Icon as IconV2 } from "@prioricode/ui/v2/icon"
import { KeybindV2 } from "@prioricode/ui/v2/keybind-v2"
import { TooltipV2 } from "@prioricode/ui/v2/tooltip-v2"
...
export function useTitlebarRightMount() {
  ...
  const sync = () => setMount(document.getElementById("prioricode-titlebar-right"))
  onMount(sync)
  ...
  return (
    <header
      ...
      classList={{
        ...
        "h-9 bg-pc-bg-deep overflow-visible": useV2Titlebar(),
        ...
        <Match when>
          ...
          >
              ...
              <div class="flex items-center gap-1 shrink-0">
                ...
                <div class="hidden xl:flex items-center shrink-0">
                  ...
                  >
                    ...
                    <div id="prioricode-titlebar-left" class="flex items-center gap-3 min-w-0 px-2" />
                  </div>
                </div>
                <ChannelIndicator debugTools={props.debugTools} />
              </div>
            ...
            <div
              ...
              data-tauri-drag-region
            >
              <div id="prioricode-titlebar-right" class="flex items-center gap-1 shrink-0 justify-end" />
              <Show when={windows()}>
...
function TitlebarV2Right(props: { state: TitlebarV2RightState }) {
  return (
    <div class="relative z-20 flex shrink-0 items-center justify-end gap-0 overflow-visible">
      ...
      <div id="prioricode-titlebar-right" class="flex shrink-0 items-center justify-end gap-0" />
    </div>
  )
}
...
function TitlebarUpdateIconButton(props: { state: TitlebarUpdatePillState }) {
  return (
    <div class="group relative mr-3 h-5 w-5 shrink-0 rounded-full bg-pc-bg-deep transition-[width] duration-150 ease-out hover:z-30 hover:w-[68px] focus-within:z-30 focus-within:w-[68px] motion-reduce:transition-none">
      <button
        type="button"
        class="absolute right-0 top-0 z-10 flex h-5 w-5 items-center justify-end overflow-hidden rounded-full bg-pc-icon-accent/20 text-pc-icon-accent transition-[width,background-color] duration-150 ease-out group-hover:w-[68px] group-hover:bg-[color-mix(in_srgb,var(--pc-icon-accent)_20%,var(--pc-bg-deep))] group-focus-within:w-[68px] group-focus-within:bg-[color-mix(in_srgb,var(--pc-icon-accent)_20%,var(--pc-bg-deep))] focus-visible:outline-none disabled:opacity-60 motion-reduce:transition-none"
        onClick={props.state.onInstall}
        disabled={props.state.installing}
        aria-busy={props.state.installing}
        aria-label={props.state.ariaLabel}
      >
        <span class="shrink-0 ml-[8px] mr-px text-[11px] text-pc-text-accent [font-weight:530] opacity-0 translate-x-2 motion-safe:transition-all duration-150 ease-out group-hover:opacity-100 group-hover:translate-x-0 group-focus-within:opacity-100 group-focus-within:translate-x-0 motion-reduce:translate-x-0">
          {props.state.label}
        </span>
...
function ChannelIndicator(props: { debugTools?: { visible: boolean; toggle: () => void } }) {
  const channel = import.meta.env.VITE_PRIORICODE_CHANNEL
