...
import { NormalizedProviderListResponse } from "@prioricode/session-ui/context"
...
import type {
  ...
  McpServer,
  SessionActiveOutput,
} from "@prioricode/client/promise"
...
export const loadLspQuery = (scope: ServerScope, directory: string, sdk: PrioricodeClient) =>
  queryOptions({
...
function makeQueryOptionsApi(
  scope: ServerScope,
  serverSDK: () => PrioricodeClient,
  serverAPI: ServerApi,
  sdkFor: (dir: PathKey) => PrioricodeClient,
  protocol: Promise<"v1" | "v2">,
) {
  return {
...
export function createServerSyncContextInner(serverSDK: ServerSDK) {
  ...
  const sdkCache = new Map<string, PrioricodeClient>()
