...
import type { retry } from "@prioricode/core/util/retry"
import type { PrioriCodeEvent, SessionApi } from "@prioricode/client/promise"
import type { Message, PrioricodeClient, Part, Session } from "@prioricode/sdk/v2/client"
...
function messageClient(...responses: Array<MessageResponse | Promise<MessageResponse>>) {
  ...
  } as unknown as PrioricodeClient
...
describe("server session", () => {
  test("projects V2 session events into current and legacy message state", () => {
    ...
    const apply = (input: object) => ctx.store.applyV2(input as PrioriCodeEvent)
  ...
  test("extends a current page to include the user for split assistant turns", async () => {
    ...
    const store = createServerSession({} as PrioricodeClient, {} as SessionApi, messageApi)
    store.remember(session("root"))
