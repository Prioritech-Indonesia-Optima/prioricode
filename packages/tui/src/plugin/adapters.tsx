import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@prioricode/plugin/tui"
...
import type { usePrioricodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof usePrioricodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getPrioricodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getPrioricodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
