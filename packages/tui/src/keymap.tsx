...
export const PRIORICODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const PRIORICODE_MODE_KEY = "prioricode.mode"

export const PrioricodeKeymapProvider = KeymapProvider
export const usePrioricodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type PrioricodeModeStack = ReturnType<typeof createPrioricodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, PrioricodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createPrioricodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(PRIORICODE_MODE_KEY, PRIORICODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(PRIORICODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(PRIORICODE_MODE_KEY, stack.at(-1)?.mode ?? PRIORICODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? PRIORICODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(PRIORICODE_MODE_KEY, undefined)
...
export function usePrioricodeModeStack() {
  return getPrioricodeModeStack(usePrioricodeKeymap())
}

export function getPrioricodeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Prioricode mode stack is not registered for this keymap")
  return value
}
...
export function registerPrioricodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createPrioricodeModeStack(keymap)
