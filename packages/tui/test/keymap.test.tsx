...
import { getPrioricodeModeStack, PRIORICODE_BASE_MODE, PrioricodeKeymapProvider, registerPrioricodeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerPrioricodeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when prioricode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getPrioricodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getPrioricodeModeStack(keymap).push("autocomplete")
