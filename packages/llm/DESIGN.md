...
> current private `@prioricode/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@prioricode/ai`
...
- Secondary audience: PrioriCode and other durable agent runtimes
...
import { LLM } from "@prioricode/ai"
import { OpenAI } from "@prioricode/ai/providers/openai"
...
```ts
import { Message } from "@prioricode/ai"
...
import { LLM, Tool } from "@prioricode/ai"
...
PrioriCode and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@prioricode/ai/promise"
import { OpenAI } from "@prioricode/ai/providers/openai"
...
```ts
import { LLMSchema } from "@prioricode/ai/schema"
...
```ts
import { Provider, Protocol } from "@prioricode/ai/provider"
...
@prioricode/ai/promise
  Promise/AsyncIterable LLM facade

@prioricode/ai/schema
  serializable domain schemas

@prioricode/ai/provider
  experimental Provider and Protocol authoring APIs

@prioricode/ai/providers/openai
@prioricode/ai/providers/anthropic
@prioricode/ai/providers/google
...
```
...
PrioriCode should migrate to `generateTurn` / `streamTurn`, preserving its durable
