...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("prioricode-find")
  api.delete("prioricode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("prioricode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("prioricode-find", new Highlight(...rest))
    return true
  }
