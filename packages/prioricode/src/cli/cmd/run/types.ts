// Shared type vocabulary for the direct interactive mode (`prioricode --mini`).
//
...
import type { PrioricodeClient, PermissionRequest, QuestionRequest, ToolPart } from "@prioricode/sdk/v2"
import type { TuiConfig } from "@prioricode/tui/config"
...
type PromptModel = Parameters<PrioricodeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<PrioricodeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<PrioricodeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<PrioricodeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<PrioricodeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<PrioricodeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<PrioricodeClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<PrioricodeClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<PrioricodeClient["question"]["reject"]>[0]
