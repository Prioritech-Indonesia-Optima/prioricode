...
//   2. Interactive local (`prioricode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`prioricode --mini --attach`): connects to a running
//      prioricode server and runs interactive mode against it.
//
...
import { createPrioricodeClient, type PrioricodeClient, type ToolPart } from "@prioricode/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<PrioricodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run prioricode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running prioricode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to PRIORICODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to PRIORICODE_SERVER_USERNAME or 'prioricode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: PrioricodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: PrioricodeClient, sessionID: string) {
      ...
      async function current(sdk: PrioricodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: PrioricodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: PrioricodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: PrioricodeClient) {
        ...
        async function loop(client: PrioricodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createPrioricodeClient({
        baseUrl: "http://prioricode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
