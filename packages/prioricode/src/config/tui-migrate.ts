...
const TUI_SCHEMA_URL = "https://prioricode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from prioricode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const prioricode = await prioricodeFiles(input)
  for (const file of prioricode) {
...
async function prioricodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "prioricode"),
    ...(await Filesystem.findUp(["prioricode.json", "prioricode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "prioricode"))
  }
  if (Flag.PRIORICODE_CONFIG) files.push(Flag.PRIORICODE_CONFIG)
