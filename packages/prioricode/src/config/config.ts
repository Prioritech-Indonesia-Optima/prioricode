...
import type { ConsoleState } from "@prioricode/core/v1/config/console-state"
import { FSUtil } from "@prioricode/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@prioricode/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@prioricode/core/v1/config/config"
import { RemoteAuthError } from "@prioricode/core/v1/config/error"
import { ConfigPermissionV1 } from "@prioricode/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@prioricode/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["prioricode.jsonc", "prioricode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://prioricode.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.PRIORICODE_CONFIG && !Flag.PRIORICODE_CONFIG_DIR && !Flag.PRIORICODE_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://prioricode.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "prioricode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "prioricode.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://prioricode.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "PRIORICODE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/prioricode`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://prioricode.ai/config.json"
        ...
        if (Flag.PRIORICODE_CONFIG) {
          yield* merge(Flag.PRIORICODE_CONFIG, yield* loadFile(Flag.PRIORICODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.PRIORICODE_CONFIG })
        }

        if (!Flag.PRIORICODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("prioricode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.PRIORICODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from PRIORICODE_CONFIG_DIR", { path: Flag.PRIORICODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".prioricode") || dir === Flag.PRIORICODE_CONFIG_DIR) {
            for (const file of ["prioricode.json", "prioricode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.prioricode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.PRIORICODE_CONFIG_CONTENT) {
          const source = "PRIORICODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.PRIORICODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from PRIORICODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["PRIORICODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("PRIORICODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.PRIORICODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.PRIORICODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("PRIORICODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.PRIORICODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.PRIORICODE_DISABLE_PRUNE) {
