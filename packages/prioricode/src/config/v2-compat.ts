...
import { NonNegativeInt, PositiveInt } from "@prioricode/core/schema"
import { ConfigAttachmentV1 } from "@prioricode/core/v1/config/attachment"
import { ConfigLSPV1 } from "@prioricode/core/v1/config/lsp"
import { InvalidError } from "@prioricode/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by PrioriCode V1. Use V1 "permission" rules or run prioricode2.',
      })),
    })
