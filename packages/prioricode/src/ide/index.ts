...
import { IdeEvent } from "@prioricode/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["PRIORICODE_CALLER"] === "vscode" || process.env["PRIORICODE_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.prioricode"], {
    nothrow: true,
  })
