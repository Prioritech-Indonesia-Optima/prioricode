...
import type { AssistantMessage, Message, PrioricodeClient, Session, SessionMessageResponse } from "@prioricode/sdk/v2"
...
export const AuthMethodID = "prioricode-login"
...
export class Service extends Context.Service<Service, Interface>()("@prioricode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `prioricode auth login` in the terminal",
      name: "Login with prioricode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: PrioricodeClient) {
...
function makeUsageService(sdk: PrioricodeClient) {
...
async function loadDirectorySnapshot(sdk: PrioricodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, prioricode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const prioricodeProvider = providers[ProviderV2.ID.make("prioricode")]
  const prioricodeModel = prioricodeProvider ? Provider.sort(Object.values(prioricodeProvider.models))[0] : undefined
  if (prioricodeProvider && prioricodeModel) return { providerID: prioricodeProvider.id, modelID: prioricodeModel.id }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "PrioriCode prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "PrioriCode service failure", service })
}
