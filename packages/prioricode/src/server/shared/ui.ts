...
export const UI_UPSTREAM = new URL("https://app.prioricode.ai")
...
export function themePreloadHash(body: string) {
  return body.match(/<script\b(?![^>]*\bsrc\s*=)[^>]*\bid=(['"])pc-theme-preload-script\1[^>]*>([\s\S]*?)<\/script>/i)
}
...
export function embeddedUI(disableEmbeddedWebUi: boolean) {
  ...
  return (embeddedUIPromise ??=
    ...
    import("prioricode-web-ui.gen.ts").then((module) => module.default as Record<string, string>).catch(() => null))
}
