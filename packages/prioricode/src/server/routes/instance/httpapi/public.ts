...
import { PrioriCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for prioricode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(PrioriCodeHttpApi)
...
export const PublicApi = PrioriCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "prioricode",
    version: "1.0.0",
    description: "prioricode api",
