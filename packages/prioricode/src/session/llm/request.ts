...
const USER_AGENT = `prioricode/${InstallationVersion}`
...
export const prepare = Effect.fn("LLMRequestPrep.prepare")(function* (input: PrepareInput) {
  ...
  const system = [
    [
      ...
      ...(input.small ? [] : [SystemPrompt.sourcing]),
      ...input.system,
  ...
  const prioricodeProjectID = input.model.providerID.startsWith("prioricode")
  ...
  return {
    ...
    headers: {
      ...(input.model.providerID.startsWith("prioricode")
        ? {
            ...(prioricodeProjectID ? { "x-prioricode-project": prioricodeProjectID } : {}),
            "x-prioricode-session": input.sessionID,
            "x-prioricode-request": input.user.id,
            "x-prioricode-client": input.flags.client,
