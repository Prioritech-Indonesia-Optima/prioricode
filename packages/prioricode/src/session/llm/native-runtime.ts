...
import type { LLMClientShape } from "@prioricode/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("prioricode"))
    return { type: "unsupported", reason: "provider is not openai, prioricode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @prioricode/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains prioricode-owned. The native runtime only adapts
      // the @prioricode/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
