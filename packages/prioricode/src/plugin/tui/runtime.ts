...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@prioricode/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@prioricode/core/effect/app-node-builder"
import { errorData, errorMessage } from "@prioricode/tui/util/error"
...
import { hasTheme, upsertTheme } from "@prioricode/tui/context/theme"
import { Global } from "@prioricode/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@prioricode/core/util/flock"
import { Flag } from "@prioricode/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@prioricode/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@prioricode/core/v1/config/plugin"
import { createCommandShim } from "@prioricode/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@prioricode/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".prioricode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".prioricode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".prioricode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.PRIORICODE_PURE ? [] : pluginOrigins
    if (Flag.PRIORICODE_PURE && pluginOrigins.length) {
    }
