...
export type PatchDeps = {
  ...
  files: (dir: string, name: "prioricode" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".prioricode")
}

function patchName(kind: Kind): "prioricode" | "tui" {
  if (kind === "server") return "prioricode"
  return "tui"
}
