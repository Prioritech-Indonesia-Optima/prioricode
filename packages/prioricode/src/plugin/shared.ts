...
export const DEPRECATED_PLUGIN_PACKAGES = ["prioricode-openai-codex-auth", "prioricode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, prioricodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(prioricodeVersion) || semver.major(prioricodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.prioricode
  if (typeof range !== "string") return
  if (!semver.satisfies(prioricodeVersion, range)) {
    throw new Error(`Plugin requires prioricode ${range} but running ${prioricodeVersion}`)
  }
}
