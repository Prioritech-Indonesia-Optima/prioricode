...
import { EventSequenceTable } from "@prioricode/core/event/sql"
...
import { WorkspaceV2 } from "@prioricode/core/workspace"
import { WorkspaceTable } from "@prioricode/core/control-plane/workspace.sql"
...
const originalEnv = {
  PRIORICODE_AUTH_CONTENT: process.env.PRIORICODE_AUTH_CONTENT,
  PRIORICODE_EXPERIMENTAL_WORKSPACES: process.env.PRIORICODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.PRIORICODE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@prioricode.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.PRIORICODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=prioricode-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.PRIORICODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.PRIORICODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.PRIORICODE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=prioricode-test")
