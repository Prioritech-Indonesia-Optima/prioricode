...
import type { HostPluginApi } from "@prioricode/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createPrioricodeClient({
  ...
  let selected = opts.theme?.selected ?? "prioricode"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "prioricode.default",
