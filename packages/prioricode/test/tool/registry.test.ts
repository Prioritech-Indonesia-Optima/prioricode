...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".prioricode")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .prioricode/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const prioricode = path.join(test.directory, ".prioricode")
      const tool = path.join(prioricode, "tool")
  ...
  it.instance("ignores non-tool exports in .prioricode/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".prioricode", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-prioricode` was registering a tool
  ...
  it.instance("loads tools from .prioricode/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const prioricode = path.join(test.directory, ".prioricode")
      const tools = path.join(prioricode, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".prioricode", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const prioricode = path.join(test.directory, ".prioricode")
        const customTools = path.join(prioricode, "tools")
        const plugin = path.join(prioricode, "node_modules", "@prioricode", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(prioricode, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@prioricode/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@prioricode/plugin"',
              "export default tool({",
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(prioricode, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@prioricode/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(prioricode, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(prioricode, "node_modules", "cowsay")
