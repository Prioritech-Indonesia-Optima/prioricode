...
const stateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      PRIORICODE_EXPERIMENTAL_WORKSPACES: Flag.PRIORICODE_EXPERIMENTAL_WORKSPACES,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.PRIORICODE_EXPERIMENTAL_WORKSPACES = original.PRIORICODE_EXPERIMENTAL_WORKSPACES
        await resetDatabase()
      }),
    )
  }),
