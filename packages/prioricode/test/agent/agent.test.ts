...
it.instance("plan agent denies edits except .prioricode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".prioricode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".prioricode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.PRIORICODE_TEST_HOME
      process.env.PRIORICODE_TEST_HOME = test.directory
