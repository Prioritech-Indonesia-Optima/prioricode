...
describe("prioricode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ prioricode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ prioricode }))
        ...
        expect(initialized.agentInfo?.name).toBe("PrioriCode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ prioricode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ prioricode })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.PRIORICODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ prioricode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("prioricode-login")
