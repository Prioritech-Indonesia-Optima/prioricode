...
describe("prioricode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // PRIORICODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ prioricode }) =>
      Effect.gen(function* () {
        const r = yield* prioricode.spawn(["mcp", "list"])
        prioricode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ prioricode }) =>
      Effect.gen(function* () {
        const r = yield* prioricode.spawn(["providers", "list"])
        prioricode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ prioricode }) =>
      Effect.gen(function* () {
        const r = yield* prioricode.spawn(["models"])
        prioricode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ prioricode }) =>
      Effect.gen(function* () {
        const r = yield* prioricode.spawn(["agent", "list"])
        prioricode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh PRIORICODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ prioricode }) =>
      Effect.gen(function* () {
        const r = yield* prioricode.spawn(["session", "list"])
        prioricode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ prioricode }) =>
      Effect.gen(function* () {
        const r = yield* prioricode.spawn(["stats"])
        prioricode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under PRIORICODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ prioricode }) =>
      Effect.gen(function* () {
        const r = yield* prioricode.spawn(["db", "path"])
        prioricode.expectExit(r, 0, "db path")
