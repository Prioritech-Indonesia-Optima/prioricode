// Subprocess integration tests for `prioricode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `prioricode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `PRIORICODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("prioricode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, prioricode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* prioricode.run("say hi")
        prioricode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, prioricode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* prioricode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, prioricode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* prioricode.run("think", { extraArgs: ["--thinking"] })
        prioricode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* prioricode.run("think again")
        prioricode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, prioricode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* prioricode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, prioricode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* prioricode.run("say hi", { format: "json" })
        prioricode.expectExit(result, 0)

        const events = prioricode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ prioricode }) =>
      Effect.gen(function* () {
        const result = yield* prioricode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, prioricode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* prioricode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, prioricode }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* prioricode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, prioricode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* prioricode.run("request permission", { permission: { bash: "ask" } })
        prioricode.expectExit(denied, 0)
        ...
        const allowed = yield* prioricode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        prioricode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* prioricode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        prioricode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, prioricode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* prioricode.serve()

        const result = yield* prioricode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, prioricode }) =>
      Effect.gen(function* () {
        const result = yield* prioricode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, prioricode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* prioricode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
