...
import { PrioricodeClient, type GlobalEvent } from "@prioricode/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<PrioricodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<PrioricodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<PrioricodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<PrioricodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<PrioricodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://prioricode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: PrioricodeClient["event"]["subscribe"]
    globalEvent?: PrioricodeClient["global"]["event"]
    promptAsync?: PrioricodeClient["session"]["promptAsync"]
    status?: PrioricodeClient["session"]["status"]
    messages?: PrioricodeClient["session"]["messages"]
    children?: PrioricodeClient["session"]["children"]
    permissions?: PrioricodeClient["permission"]["list"]
    questions?: PrioricodeClient["question"]["list"]
  } = {},
) {
  ...
  const status: PrioricodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: PrioricodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: PrioricodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: PrioricodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: PrioricodeClient["question"]["list"] = input.questions ?? (() => ok([]))
