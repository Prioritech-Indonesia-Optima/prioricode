...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ prioricode }) =>
    Effect.gen(function* () {
      const result = yield* prioricode.spawn(["--replay-limit", "10"])

      prioricode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ prioricode }) =>
    Effect.gen(function* () {
      const result = yield* prioricode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ prioricode }) =>
    Effect.gen(function* () {
      const result = yield* prioricode.spawn(["--mini", "--port", "4096"])
