...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "prioricode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "prioricode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "prioricode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "prioricode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["prioricode.json", "prioricode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "prioricode.json"),
        path.join(project, "prioricode.jsonc"),
      ])
    })
  })
