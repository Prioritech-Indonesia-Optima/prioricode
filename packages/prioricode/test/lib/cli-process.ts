// Subprocess test harness for the prioricode CLI. Spawns the real binary against
...
//
// Configuration flows through prioricode's built-in test affordances:
//   - PRIORICODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - PRIORICODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - PRIORICODE_DISABLE_PROJECT_CONFIG : skip walking up for prioricode.json
//   - PRIORICODE_PURE                : skip external plugin discovery + install
//   - PRIORICODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `prioricode.run` is fully wired. The shape supports adding more
// builders (`prioricode.serve(opts)`, `prioricode.acp(opts)`, `prioricode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of PrioricodeCli.
...
const prioricodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(prioricodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    PRIORICODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    PRIORICODE_CONFIG_CONTENT: configJson,
    PRIORICODE_DISABLE_PROJECT_CONFIG: "1",
    PRIORICODE_PURE: "1",
    PRIORICODE_DISABLE_AUTOUPDATE: "1",
    PRIORICODE_DISABLE_AUTOCOMPACT: "1",
    PRIORICODE_DISABLE_MODELS_FETCH: "1",
    PRIORICODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `prioricode run`. New flags should
...
// `prioricode serve` is a long-lived process — it never exits on its own.
...
// `prioricode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type PrioricodeCli = {
  ...
  // Spawn `prioricode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `prioricode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly prioricode: PrioricodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("prioricode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          PRIORICODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("prioricode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("prioricode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "prioricode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `prioricode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("prioricode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const prioricode: PrioricodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, prioricode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `prioricode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "prioricode") {
...
// every call site. Use this for any test that needs the prioricode CLI fixture.
//
...
// resources (e.g. `prioricode.serve`) without an extra `Effect.scoped` wrapper —
