...
import { LLMEvent, ToolFailure } from "@prioricode/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@prioricode/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("prioricode"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("prioricode") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("prioricode") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, prioricode, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored prioricode auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("prioricode") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("prioricode"),
