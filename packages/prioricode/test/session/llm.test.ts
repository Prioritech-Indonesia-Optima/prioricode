...
import { LLMClient, RequestExecutor } from "@prioricode/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const prioricodeFixture = { providerID: "prioricode-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for prioricode providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(prioricodeFixture.providerID),
          ModelV2.ID.make(prioricodeFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(prioricodeFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [prioricodeFixture.providerID],
          provider: {
            [prioricodeFixture.providerID]: {
              name: "PrioriCode Test",
