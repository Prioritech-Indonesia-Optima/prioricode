...
import { ModelsDev } from "@prioricode/core/models-dev"
...
import { LLMEvent, LLMResponse } from "@prioricode/llm"
import { RequestExecutor } from "@prioricode/llm/route"
...
const zenURL = (connection: string) => `https://console.prioricode.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.PRIORICODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["prioricode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("PRIORICODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("PRIORICODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["prioricode", "native", "oauth", "tool-loop"],
  ...
  {
    id: "prioricode-proxy",
    name: "PrioriCode proxy",
    providerID: ProviderV2.ID.prioricode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["prioricode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.PRIORICODE_RECORD_CONSOLE_TOKEN && process.env.PRIORICODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.prioricode,
        name: "PrioriCode Zen",
        env: ["PRIORICODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.PRIORICODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.PRIORICODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.PRIORICODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["prioricode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("PRIORICODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("PRIORICODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("PRIORICODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set PRIORICODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the prioricode LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "prioricode.json"),
      JSON.stringify({ $schema: "https://prioricode.ai/config.json", ...scenario.config(model) }),
    ),
  )
