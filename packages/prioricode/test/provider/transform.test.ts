    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".prioricode/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const prioricodeModel = {
      ...openaiModel,
      providerID: "prioricode",
      api: {
        id: "prioricode-test",
        url: "https://api.prioricode.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, prioricodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.prioricode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.prioricode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          prioricode: { itemId: "msg_prioricode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              prioricode: { itemId: "msg_prioricode_part" },
    ...
    expect(result[0].providerOptions?.prioricode?.itemId).toBe("msg_prioricode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.prioricode?.itemId).toBe("msg_prioricode_part")
...
describe("ProviderTransform sampling defaults - DeepSeek", () => {
  ...
  test.each([
    ...
    ["prioricode", "deepseek-v4-flash"],
    ["prioricode-go", "deepseek-v4-flash"],
