...
Delete `packages/prioricode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/prioricode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/prioricode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/prioricode`
- TUI plugin loader tests in `packages/prioricode` if runtime plugin API wiring changed
