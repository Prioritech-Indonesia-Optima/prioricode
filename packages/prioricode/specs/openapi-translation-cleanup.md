...
Trim `packages/prioricode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/prioricode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/prioricode`.
- `[x]` `bun typecheck` from `packages/prioricode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/prioricode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/prioricode`.
- `[x]` `bun dev generate > /tmp/prioricode-openapi.json` from `packages/prioricode`.
...
- Edit `packages/prioricode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/prioricode-openapi.json` from `packages/prioricode`.
...
- Check source schemas in files like `packages/prioricode/src/session/schema.ts`, `packages/prioricode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/prioricode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/prioricode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
