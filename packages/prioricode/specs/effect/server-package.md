...
Practical reference for a future `packages/server` split after the prioricode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/prioricode`.
...
Until enough shared service code lives outside `packages/prioricode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/prioricode`

It should not import `packages/prioricode` services while `packages/prioricode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on prioricode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/prioricode` implementation details.
