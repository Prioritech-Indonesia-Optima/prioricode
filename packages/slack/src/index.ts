...
import { createPrioricode, type ToolPart } from "@prioricode/sdk"
...
console.log("🚀 Starting prioricode server...")
const prioricode = await createPrioricode({
  port: 0,
})
console.log("✅ Prioricode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await prioricode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new prioricode session...")
    const { client, server } = prioricode
    ...
    console.log("✅ Created prioricode session:", createResult.data.id)
  ...
  console.log("📝 Sending to prioricode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Prioricode response:", JSON.stringify(result, null, 2))
