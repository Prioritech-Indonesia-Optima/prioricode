...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@prioricode/acme")).toBe("@prioricode/acme")
    expect(Npm.sanitize("@prioricode/acme@1.0.0")).toBe("@prioricode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/prioricode/acme.git"
    const expected = win ? "acme@git+https_//github.com/prioricode/acme.git" : spec
