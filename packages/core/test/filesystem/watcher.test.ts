...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    PRIORICODE_EXPERIMENTAL_FILEWATCHER: "true",
    PRIORICODE_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@prioricode.test`.cwd(tmp.path).quiet()
