...
import { PluginHost } from "@prioricode/core/plugin/host"
import { PrioricodePlugin } from "@prioricode/core/plugin/provider/prioricode"
...
const addPlugin = Effect.fn(function* (http?: HttpClient.HttpClient) {
  ...
  yield* PrioricodePlugin.effect(host).pipe(
...
describe("PrioricodePlugin", () => {
  it.effect("registers account and service account methods", () =>
    Effect.gen(function* () {
      yield* addPlugin()
      expect((yield* (yield* Integration.Service).get(Integration.ID.make("prioricode")))?.methods).toEqual([
        {
          id: Integration.MethodID.make("device"),
          type: "oauth",
          label: "PrioriCode Console account",
        },
  ...
  it.effect("resolves origin-rooted device verification URLs", () =>
    Effect.gen(function* () {
        ...
        Effect.succeed(
          HttpClientResponse.fromWeb(
            ...
            Response.json({
              device_code: "device",
              user_code: "user",
              verification_uri_complete: "/console/device?user_code=user&client_id=prioricode-cli",
              expires_in: 60,
              interval: 60,
            }),
          ),
      ...
      const attempt = yield* integration.connection.oauth({
        integrationID: Integration.ID.make("prioricode"),
        methodID: Integration.MethodID.make("device"),
        inputs: {},
      })
      expect(attempt.url).toBe("https://prioricode.ai/console/device?user_code=user&client_id=prioricode-cli")
    }),
  )
  ...
  it.live("loads providers and models from the connected PrioriCode server", () =>
        ...
        Effect.gen(function* () {
          ...
          const provider = required(
            yield* eventually(
              ...
              (item) => item?.integrationID === Integration.ID.make("prioricode"),
            ),
          )
          expect(provider).toMatchObject({
            name: "Remote",
            integrationID: "prioricode",
            api: {
              type: "aisdk",
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ PRIORICODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
            ...ProviderV2.Info.empty(ProviderV2.ID.prioricode),
  ...
  it.effect("keeps free models without credentials", () =>
    withEnv({ PRIORICODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.prioricode)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.prioricode, ModelV2.ID.make("free"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("uses PRIORICODE_API_KEY as credentials", () =>
    withEnv({ PRIORICODE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.prioricode)).request.body.apiKey).toBeUndefined()
        expect(required(yield* catalog.model.get(ProviderV2.ID.prioricode, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ PRIORICODE_API_KEY: undefined, CUSTOM_PRIORICODE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("prioricode"),
            method: { type: "env", names: ["CUSTOM_PRIORICODE_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured apiKey as credentials", () =>
    withEnv({ PRIORICODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.prioricode)).request.body.apiKey).toBe("configured")
        expect(required(yield* catalog.model.get(ProviderV2.ID.prioricode, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("ignores non-prioricode providers and models", () =>
    withEnv({ PRIORICODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
  ...
  it.effect("prefers gpt-5-nano as the prioricode small model", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
      const providerID = ProviderV2.ID.prioricode
