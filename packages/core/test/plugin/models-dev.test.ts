...
import { ModelsDevPlugin } from "@prioricode/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.PRIORICODE_MODELS_PATH,
          disabled: Flag.PRIORICODE_DISABLE_MODELS_FETCH,
        }
        Flag.PRIORICODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.PRIORICODE_MODELS_PATH = previous.path
          Flag.PRIORICODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
