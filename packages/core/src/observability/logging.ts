...
export function fileLogger(file = path.join(Global.Path.log, "prioricode.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.PRIORICODE_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.PRIORICODE_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
