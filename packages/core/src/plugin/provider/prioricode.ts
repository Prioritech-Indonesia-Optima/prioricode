...
import type { IntegrationOAuthMethodRegistration } from "@prioricode/plugin/v2/effect/integration"
import { define } from "@prioricode/plugin/v2/effect/plugin"
import type { CredentialValue } from "@prioricode/sdk/v2/types"
...
const defaultServer = "https://prioricode.ai/console"
const clientID = "prioricode-cli"
...
export const PrioricodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "prioricode",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("PrioricodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("prioricode")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load PrioriCode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("prioricode", (integration) => {
        integration.name = "PrioriCode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "prioricode", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("prioricode")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("prioricode")
      ...
      const item = catalog.provider.get(ProviderV2.ID.prioricode)
      if (!item) return
      const hasKey = Boolean(process.env.PRIORICODE_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("prioricode")),
