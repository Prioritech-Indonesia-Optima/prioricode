...
// OAuth Client ID Metadata Document for the prioricode client.
// Spec: https://datatracker.ietf.org/doc/draft-ietf-oauth-client-id-metadata-document/
//
// When an MCP server's authorization server supports this, prioricode sends this URL as its OAuth client_id
...
// built from the request origin and stays valid on dev.prioricode.ai as well as production.
//
// redirect_uris have no port because prioricode binds an ephemeral port per login. RFC 8252 section 7.3 has
// authorization servers ignore the port when matching loopback redirects for native apps.
const PATH = "/oauth/prioricode/client.json"
