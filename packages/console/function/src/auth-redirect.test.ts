...
describe("authorization redirect validation", () => {
  test("allows registered PrioriCode callbacks", () => {
    expect(isAllowedAuthorizationRedirect("app", "https://prioricode.ai/auth/callback")).toBe(true)
    expect(isAllowedAuthorizationRedirect("app", "https://dev.prioricode.ai/auth/callback")).toBe(true)
  ...
  test("rejects unregistered clients and external redirects", () => {
    expect(isAllowedAuthorizationRedirect("other", "https://prioricode.ai/auth/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://evil.example/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://prioricode.ai.evil.example/callback")).toBe(false)
