# Contributing to PrioriCode

We want to make it easy for you to contribute to PrioriCode. Here are the most common type of changes that get merged:
...
- [`help wanted`](https://github.com/Prioritech-Indonesia-Optima/prioricode/issues?q=is%3Aissue%20state%3Aopen%20label%3Ahelp-wanted)
- [`good first issue`](https://github.com/Prioritech-Indonesia-Optima/prioricode/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22good%20first%20issue%22)
- [`bug`](https://github.com/Prioritech-Indonesia-Optima/prioricode/issues?q=is%3Aissue%20state%3Aopen%20label%3Abug)
- [`perf`](https://github.com/Prioritech-Indonesia-Optima/prioricode/issues?q=is%3Aopen%20is%3Aissue%20label%3A%22perf%22)
...
To run PrioriCode in the root of the prioricode repo itself:
...
During development, `bun dev` is the local equivalent of the built `prioricode` command. Both run the same CLI interface:
...
# Production
prioricode --help          # Show all available commands
prioricode serve           # Start headless API server
prioricode web             # Start server + open web interface
prioricode <directory>     # Start TUI in specific directory
```

### Running the API Server

To start the PrioriCode headless API server:
...
1. **First, start the PrioriCode server** (see [Running the API Server](#running-the-api-server) section above)
...
The most reliable way to debug PrioriCode is to run it manually in a terminal via `bun run --inspect=<url> dev ...` and attach
...
- The same problem arises when running PrioriCode in the VSCode `JavaScript Debug Terminal`
...
- `chore(prioricode):` maintenance in the prioricode package
...
For net-new functionality, start with a design conversation. Open an issue describing the problem, your proposed approach (optional), and why it belongs in PrioriCode. The core team will help decide whether it should move forward; please wait for that approval instead of opening a feature PR directly.
